...
| `prepare_reasoning_catalog(*, api_key, base_url)` | Called before inference with the resolved route credentials. May load a catalog and return a route-bound copy of the profile; must not mutate the registry singleton. Default returns self. |
