...
def _session_is_lru_evictable(sid: str, session: dict) -> bool:
    ...
    work, terminal processes/results, or on a live transport. Lazy watch sessions never start a build, so their unset agent_ready must not
    make them immortal."""
    if (session.get("running") or session.get("queued_prompt") or session.get("queued_prompts")
            or session.get("_auto_continue_scheduled") or _session_pending_kind(sid)
            or _session_has_background_processes(session) or _session_has_active_delegations(sid, session)):
        return False
