...
let
  ...
  # so npmRoot must be the original flake source rather than a lazy
  # fileset.toSource path: the latter is a derivation output that need not
  # exist yet on a cold read-only store, which breaks evaluation there.
  # Rebuild scoping is unaffected — the manifests are read per-file at eval
  # time, so the npmDeps derivation still depends only on manifest contents,
