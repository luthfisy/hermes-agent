...
export default function WebhooksPage() {
  ...
  const handleRestart = useCallback(async () => {
    ...
    try {
      ...
      showToast(t.webhooks.restartFailed.replace("{error}", errorMessage(e)), "error");
    } finally {
      setRestarting(false);
    }
  ...
  const handleEnableWebhooks = useCallback(async () => {
    ...
    try {
      ...
      showToast(
        t.webhooks.enableFailed.replace("{error}", errorMessage(e)),
        "error",
      );
    } finally {
      setEnabling(false);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast(t.webhooks.createFailed.replace("{error}", errorMessage(e)), "error");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleToggleEnabled = useCallback(
    async (subName: string, nextEnabled: boolean) => {
      ...
      try {
        ...
        showToast(
          nextEnabled
            ? t.webhooks.enabled.replace("{name}", subName)
            : t.webhooks.disabled.replace("{name}", subName),
          "success",
        );
        loadWebhooks();
      } catch (e) {
        showToast(
          t.webhooks.error.replace("{error}", errorMessage(e)),
          "error",
        );
      } finally {
    ...
    [loadWebhooks, showToast, t.webhooks.enabled, t.webhooks.disabled, t.webhooks.error],
  );
    ...
    onDelete: useCallback(
      async (name: string) => {
        try {
          ...
          showToast(t.webhooks.deleted.replace("{name}", name), "success");
          loadWebhooks();
        } catch (e) {
          showToast(
            t.webhooks.error.replace("{error}", errorMessage(e)),
      ...
      [loadWebhooks, showToast, t.webhooks.deleted, t.webhooks.error],
    ),
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <DeleteConfirmDialog
        ...
        description={
          pendingName
            ? t.webhooks.deleteDescription.replace("{name}", pendingName)
            : t.webhooks.deleteDescriptionPlain
        }
        ...
        >
          <div className={cn(themedBody, "relative w-full max-w-lg border border-border bg-card shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto")}>
            ...
            {created ? (
              ...
              <div className="p-5 grid gap-4">
                <div className="grid gap-2">
                  <Label htmlFor="webhook-name">{t.fields.name}</Label>
                  <Input
                    id="webhook-name"
                    autoFocus
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label htmlFor="webhook-deliver">{t.fields.deliverTo}</Label>
                    <Select
                      id="webhook-deliver"
                      value={deliver}
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="webhook-deliver-only">{t.fields.deliverOnly}</Label>
                ...
                <div className="grid gap-2">
                  <Label htmlFor="webhook-prompt">{t.fields.prompt}</Label>
                  <textarea
                    id="webhook-prompt"
