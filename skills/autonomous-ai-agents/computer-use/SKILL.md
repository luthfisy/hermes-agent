...
Detection scope: `auto` queries the *Windows* session of the Hermes
process itself (`ProcessIdToSessionId`), so it fires on a Windows gateway
in Session 0 regardless of who is logged in at the console. It cannot
fire on a non-Windows host: a WSL2 Hermes that spawns a Windows
`cua-driver.exe` through interop is still a POSIX process with no way to
read the child's Windows session, so detection is "unknown" and `auto`
stays on MCP. If that WSL host is a *system* service (its Windows
children land in Session 0 and hit the same rejection), set
`computer_use.session0_transport: cli` explicitly — the escape hatch is
