...
- V1 若允许 `codex_app_server` 作为主 runtime，plugin policy 会被旁路；因此必须限制该模式或把它作为独立 delegated runtime。
...
- V1 不把 `codex_app_server` 用作受 Engineering lifecycle 管理的主 runtime，因为它旁路正常 tool hooks；若启用，只能作为明确标记的 delegated runtime，并且其结果仍需外层 verification/review。
...
- Codex app-server 和外部 CLI worker 需要明确隔离，不能假装完全受 Hermes tool hooks 控制。
...
| Codex/Claude Code/OpenCode | 区分 provider、Hermes subagent、app-server、skill+subprocess；不统一误分类 |
