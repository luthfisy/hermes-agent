...
def _recover_user_raw(config_path: Path, path_key: str, exc: Exception) -> Dict[str, Any]:
    """Last-known-good raw user mapping after a load failure: this process's last good parse,
    else the newest readable ``good`` copy in backups/config/, else ``{}`` (warned as defaults).
    Only a real parse failure is reported as broken YAML and quarantined; any other fault is logged
    with its true type and leaves the file alone (``hermes_cli.config._report_config_load_failure``)."""
    ...
    _config._report_config_load_failure(
...
def load_user_config_effective(config_path: Optional[Path] = None, *, fail_closed: bool = False) -> Dict[str, Any]:
    ...
    Re-entrant call (this thread is already inside a config load, e.g. from a log handler that
    renders a warning emitted inside the load) returns that load's in-flight mapping instead of
    re-parsing — see ``hermes_cli.config._LOAD_STATE``.
    """
    if config_path is None:
        config_path = _config.get_config_path()
    path_key = str(config_path)
    reentrant = _config._reentrant_config(path_key)
    if reentrant is not None:
        return reentrant
    with _config._CONFIG_LOCK:
        _config._begin_config_load()
        try:
            ...
            if user_sig is not None and raw_hit is not None and raw_hit[:4] == user_sig:
                ...
                try:
                    with open(config_path, encoding="utf-8") as f:
                        raw = _config._require_mapping_root(fast_safe_load(f))
                    ...
                    # A re-entrant read on this thread gets the user layer rather than a re-parse.
                    _config._publish_in_flight_config(raw)
