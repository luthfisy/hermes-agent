...
def build_auth_parser(subparsers, *, cmd_auth: Callable) -> None:
    ...
    auth_rename = auth_subparsers.add_parser(
        "rename", help="Rename a pooled credential's label")
    auth_rename.add_argument("provider", help="Provider id")
    auth_rename.add_argument("target", help="Credential index, entry id, or exact current label")
    auth_rename.add_argument("new_label", help="New label (non-empty, no control characters)")
