...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        # Suppress the warning (NOT the placeholder) while a .env this process has not read yet
        # exists on disk. Several entrypoints import this module — expanding config refs as an
        # import side effect — before calling load_hermes_dotenv(), so a var that lives only in
        # .env looks unset here and printed a false "is not set" line on every CLI invocation.
        # The returned value is unchanged: load_config()'s env-ref snapshot re-expands once the
        # environment actually changes (#58514). A genuinely-missing var still warns, because by
        # then the .env has been loaded.
