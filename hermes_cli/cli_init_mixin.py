...
class CLIInitMixin:
    ...
    def _init_toolsets(self, toolsets):
        ...
        if toolsets and "all" not in toolsets and "*" not in toolsets:
            # MCP server names only resolve after discover_mcp_tools runs; skip them here. The exempted
            # names must come from the same resolver the platform merge consults, or plugin-shipped
            # (portable) MCP servers -- whose names exist only in memory -- warn on every launch (#119457).
            try:
                from hermes_cli.tools_config import enabled_mcp_server_names
                mcp_names = enabled_mcp_server_names(CLI_CONFIG)
            except Exception:
