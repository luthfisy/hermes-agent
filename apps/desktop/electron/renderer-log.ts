...
/** Format Electron's `console-message` event into one line, or null for
 *  non-error levels. `level` is numeric 0..3, where 3 === error. */
export function formatRendererConsoleLine(label: string, event: ConsoleMessageEvent): string | null {
  if (event.level !== 3) {
    return null
  }

  return `[renderer console:${label}] ${String(event.message)} (${String(event.sourceUrl)}:${String(event.lineNumber)})`
}
