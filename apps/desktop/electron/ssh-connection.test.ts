...
// ControlMaster muxing is disabled by design on Windows (ssh-connection.ts
// `_mux = opts.mux ?? process.platform !== 'win32'`), so every test below that
// pins POSIX control-socket/master semantics must gate on it.
