 ...
 * `analysis` stays out of public text: it is private scratchpad narration.
 */
...
/**
 * The Responses adapter (`_OutputScan._message`) flattens public
 * `phase=commentary` text into the persisted `reasoning` blob beside genuine
 * reasoning summaries. The sidecar keeps the exact channel, so hydration can
 * lift the public sentence back out: strip every sidecar commentary occurrence
 * from the reasoning text (whitespace-tolerant) and keep private reasoning.
 * Source rows and provider replay are never mutated — this is a read-only
 * display projection.
 */
function stripCommentaryFromReasoning(reasoning: string, commentary: string[]): string {
...
export function toChatMessages(messages: SessionMessage[]): ChatMessage[] {
  ...
  messages.forEach((message, index) => {
    ...
    // Same sentence may live in both the sidecar (public) and the reasoning
    // blob (flattened by the adapter). Keep it in exactly one channel: text.
    const reasoning = codexText ? stripCommentaryFromReasoning(rawReasoning, codexText.commentary) : rawReasoning
