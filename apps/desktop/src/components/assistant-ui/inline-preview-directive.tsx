 ...
 * gated to this frame. Structured widget payloads are delivered intact; an
 * invalid widget argument is rejected through `send()` returning `false`.
 *
 ...
 *  routes it through the composer as a normal user message —
...
export function intentScript(token: string): string {
  return (
    ...
    ',token:t,prompt:p},"*");return true}' +
    'window.hermes={canSend:true,send:send};' +
 ...
 *  this one turns into a user message. Trimmed but never silently modified. */
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (intent !== null) {
        ...
        // updating IS the visible response. Do not add a local throttle here:
        // dropping a second structured intent loses user data.
