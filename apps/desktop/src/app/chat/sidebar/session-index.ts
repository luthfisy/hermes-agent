 ...
 *
 * The sidebar fetches four independent slices — recents, cron, messaging, and
 * kanban — and renders the latter three in self-managed sections. Any of them
 * can be pinned, so all four must be indexed here or the Pinned section can't
...
export function buildSessionByAnyId(
  ...
  kanbanSessions: SessionInfo[] = []
): Map<string, SessionInfo> {
  const map = new Map<string, SessionInfo>()

  for (const session of [...cronSessions, ...messagingSessions, ...kanbanSessions, ...visibleSessions]) {
