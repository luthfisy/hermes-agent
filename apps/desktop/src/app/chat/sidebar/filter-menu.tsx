...
function createMenuOptions(copy: Translations['sidebar']['filterMenu']) {
  ...
  const groupingOptions: Record<SidebarGrouping, Omit<Option<SidebarGrouping>, 'id'>> = {
    date: { icon: 'clock', label: options.updated },
    profile: { icon: 'account', label: options.profile },
    project: { icon: 'root-folder', label: options.project },
    status: { icon: 'pulse', label: options.status }
  }

  const groupings: Option<SidebarGrouping>[] = SIDEBAR_GROUPING_ORDER.map(id => ({
    id,
    ...groupingOptions[id]
  }))
