...
export const CatalogBrowser = memo(function CatalogBrowser({
  ...
  const selectedId = selectedEntryId === undefined ? localSelectedId : selectedEntryId
  ...
  const filtered = useMemo(
    () =>
      entries.filter(
        entry =>
          (view === 'installed' || source === 'all' || entry.source === source) &&
          (view === 'installed' || category === 'all' || entry.category === category) &&
          (!deferredQuery || entry.search.includes(deferredQuery))
      ),
    [entries, source, category, deferredQuery, view]
  )
  ...
  useEffect(() => {
    ...
    setDetailOpen(Boolean(selectedEntryId))
  }, [deferredQuery, selectedEntryId])
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col" data-catalog={kind}>
      ...
      <div className="min-h-0 flex-1">
        ...
        ) : view === 'browse' && error && !entries.length ? (
