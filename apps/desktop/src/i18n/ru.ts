...
export const ru = defineLocale({
  ...
  settings: {
    ...
    telemetry: {
      title: 'Телеметрия',
      intro:
        'Hermes может собирать анонимные метрики использования на этом компьютере и делиться ими с Nous Research, чтобы улучшать агента. Выключено, пока вы не включите.',
      pointCounters:
        'Только ограниченные счётчики и гистограммы: вызовы модели, использование инструментов, ошибки, интервалы задержки.',
      pointNoContent: 'Никогда — промпты, ответы, содержимое файлов, имена файлов или что-либо введённое в чат.',
      pointInstallId:
        'Пакеты несут случайный ID установки для каждого профиля, чтобы группировать дни. Он не содержит личных данных и сбрасывается при удалении папки телеметрии.',
      pointConsentWindow:
        'Дневной пакет отправляется, только если весь период его сбора попадает во время, когда обмен был включён. Данные до согласия или из любого перерыва остаются на этом компьютере.',
      docsLink: 'Прочитать полное описание данных',
      sharingTitle: 'Обмен',
      shareLabel: 'Делиться анонимными метриками использования с Nous Research',
      shareDesc:
        'Собирает метрики локально и отправляет дневной пакет в Nous. Выключение немедленно останавливает отправку.',
      collectOnlyNote:
        'Этот профиль собирает метрики локально, но не отправляет их (настроено из командной строки). Включение обмена начнёт отправку; выключение остановит и сбор.',
      inheritedNote:
        'У этого профиля нет собственной настройки; он следует ответу, данному при первом вопросе. Переключение задаёт настройку только для этого профиля.',
      deploymentNote:
        'У этого профиля нет собственной настройки; он следует значению по умолчанию, заданному этим развёртыванием. Переключение задаёт настройку только для этого профиля.',
      cliHint: 'Та же настройка доступна в терминале через `hermes setup telemetry`.',
      enabledTitle: 'Обмен телеметрией включён',
      enabledMessage: 'Метрики собираются локально и ежедневно отправляются в Nous Research.',
      disabledTitle: 'Обмен телеметрией выключен',
      disabledMessage: 'Ничего не собирается и не отправляется. Существующие локальные файлы оставлены на месте.',
      failedSave: 'Не удалось сохранить настройку телеметрии',
      filesTitle: 'Локальные файлы',
      filesLabel: 'Папка телеметрии',
      filesDesc:
        'Всё, что собирает Hermes, находится здесь: база метрик, дневные пакеты в очереди на отправку и запись о том, когда обмен был включён. Удалите папку, чтобы сбросить ID установки.',
      filesRemoteDesc: 'Этот профиль работает на удалённом шлюзе; его папка телеметрии находится на той машине.',
      openFolder: 'Открыть папку',
      openFolderFailed: 'Не удалось открыть папку телеметрии',
      promptTitle: 'Делиться анонимными метриками использования?',
      promptDescription:
        'Hermes может отправлять в Nous Research ограниченные счётчики использования (вызовы модели, использование инструментов, ошибки) — никогда не промпты, файлы или личные данные. Это можно изменить в любой момент в Настройки → Телеметрия.',
      promptAccept: 'Делиться',
      promptDecline: 'Не делиться'
    },
    notifications: {
      title: 'Уведомления',
  ...
  onboarding: {
    ...
    shareMetricsLabel: 'Делиться анонимными метриками использования с Nous Research',
