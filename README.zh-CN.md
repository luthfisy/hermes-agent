...
Hermes 的几个名词容易混在一起。可以先按下面的层次理解：

| 概念 | 含义 | 常用场景 |
|------|------|----------|
| **Session（会话）** | 一次对话的历史记录和上下文。 | 继续上次任务、回看工具输出、压缩长对话。 |
| **Project / Workspace（项目 / 工作区）** | 当前任务所在的目录，以及其中的 `AGENTS.md`、源码、文档和命令。 | 让 Hermes 理解一个代码库或知识库的本地规则。 |
| **Profile（配置档）** | 一套隔离的 Hermes 身份：独立配置、记忆、技能、会话和平台设置。 | 把工作、研究、社交账号或不同机器人分开，避免记忆和权限串线。 |
| **Bot（机器人）** | 在桌面端或消息平台里对外呈现的 profile，可以有自己的名字、头像和职责。 | 为不同任务创建专门的助理，例如研究员、运维员、写作员。 |
| **Skill（技能）** | 可复用的过程记忆，通常是 `SKILL.md` 加脚本、模板或参考资料。 | 把已经跑通的流程固化下来，下次同类任务自动加载。 |
| **Plugin（插件）** | 扩展 Hermes 能力的代码包。能力优先放在插件、MCP 或技能里，而不是塞进核心工具集。 | 接入新服务、添加平台能力、扩展仪表盘或自动化流程。 |

一个实用判断：**session 是一次工作，workspace 是工作现场，profile 是一个独立身份，bot 是这个身份的外部形态，skill 是经验，plugin 是能力扩展。**
