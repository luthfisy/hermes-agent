...
from tools.terminal_tool_guards import (
    ...
    bash_dev_tcp_redirect_block, gateway_lifecycle_block, self_repo_block,
)
...
def _pre_exec_block(
    ...
    Order matters: the Bash /dev/tcp crash guard first (before a shell can be
    spawned), then gateway lifecycle, dangerous-workdir, and self-repo guards.
    """
    blocked = bash_dev_tcp_redirect_block(command)
    if blocked:
