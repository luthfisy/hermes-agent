...
def _resolve_mistral_context_bias() -> list:
    """Voxtral vocabulary bias: ``stt.mistral.context_bias`` then ``stt.context_bias``.

    Each entry must be a single token: the API answers 400 to any item holding a space or a comma.
    A multi-word entry is therefore split into its tokens here rather than sent as-is — left alone,
    one misconfigured line would cost a refused request plus a retry on *every* transcription."""
    from tools.transcription_tools import _load_stt_config
    ...
    for source in (_get_stt_section(stt_config, "mistral"), stt_config):
        ...
        for entry in raw:
            ...
            if len(tokens) > 1 and str(entry) not in _BIAS_SPLIT_WARNED:
                # Once per distinct entry: a misconfiguration deserves to be seen, not to be
                # repeated on every voice message.
                _BIAS_SPLIT_WARNED.add(str(entry))
                logger.warning("STT context_bias entry %r is not a single token — sending it as %s instead. "
                               "Split it in the config to silence this.", entry, tokens)
            terms.extend(tokens)
        if terms:
            return terms
    return []
...
def _may_be_context_bias_rejection(err: Exception) -> bool:
    ...
    A candidate test, not a verdict. The server names the vocabulary only sometimes, so a bare 400
    has to qualify too — which means this alone cannot tell a refused vocabulary from an unrelated
    bad request. What settles it is the retry itself, in :func:`_transcribe_mistral`. 5xx, timeouts
    and auth errors never qualify."""
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # It fails without the vocabulary too, so the vocabulary was not the problem: report
                # the original error, and never claim the operator's configuration was at fault.
                raise bias_err from None
            # Only now is the diagnosis earned — the same request succeeded once the vocabulary
            # was dropped.
            logger.warning("Mistral refused the transcription vocabulary (%d terms) — transcribed without it: %s",
