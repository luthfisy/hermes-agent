...
class GatewayInboundMixin:
    ...
    async def _hm_admit_event(
        ...
        if not getattr(event, "_bot_loop_admitted", False) and not self._admit_bot_message_for_source(source_for_event_actor(event)):
            return None
    ...
    async def _hm_command_hooks(
        ...
        hook_ctx = {
            "platform": platform, "user_id": source_for_event_actor(event).user_id, "command": canonical,
