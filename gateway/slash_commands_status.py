...
class GatewayStatusCommandsMixin:
    ...
    async def _persisted_billing_route(self, source):
        ...
        SessionDB row / most recent route, when no agent is resident."""
        async def _rows():
        ...
        entry, persisted, recent = await _quiet(_rows, (None, {}, {}))
        # A /model switch evicts the cached agent, so between turns the recent
