...
# config that were authoritative at ENQUEUE time, not whatever the provider
# holds when the writer thread eventually drains the queue.
# ---------------------------------------------------------------------------


class TestRetainIdentityIsolation:
    """Regression for the enqueue/run-time identity gap.

    ``_make_turn_retain_job`` snapshots turns/metadata/lineage/bank_id at enqueue
    time, but the writer job still calls ``_build_retain_kwargs`` at run time,
    which re-reads mutable per-session attributes (``_retain_tags``,
    ``_observation_scopes``, ``_retain_source``). A session switch in between
    stamps a queued OLD-session retain with the NEW session's item config.
    """

    def _gated_provider(self, provider_with_config):
        p = provider_with_config(
            retain_tags=["old-tag"],
        ...
        p._retain_queue.put(_gate)
        return p, gate, released

    def test_queued_retain_keeps_enqueue_time_item_config(self, provider_with_config):
        p, gate, released = self._gated_provider(provider_with_config)
        p.sync_turn("old-user", "old-assistant")
        gate.wait(timeout=5.0)

        # Mutate the item-config attributes the writer must NOT observe.
        p._retain_tags = ["new-tag"]
        p._observation_scopes = "all_combinations"
        ...
        assert item["tags"] == ["old-tag", "session:test-session"]
