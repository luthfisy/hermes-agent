...
class TestModelStateIncludesNamedProviders:
    ...
    @pytest.mark.parametrize(
        ("current_provider", "requested_provider", "inventory_provider", "current_model", "expected_current_id"),
        [
            ("openai-codex", None, "9router", "gpt-5.4", "openai-codex:gpt-5.4"),
            ("custom", "custom:9router", "custom", "tflow/gpt-5.6-sol", "custom:9router:tflow/gpt-5.6-sol"),
        ],
    )
    def test_named_provider_inventory_slug_uses_canonical_choice_id(
        self, monkeypatch, tmp_path, current_provider, requested_provider, inventory_provider,
        current_model, expected_current_id,
    ):
        import yaml
        from hermes_cli.models import parse_model_input
        ...
        config = {
            ...
            "providers": {
                "9router": {
                    "name": "9Router", "base_url": "https://router.example/v1", "api_key": "x",
        ...
        manager = SessionManager(
            agent_factory=lambda: SimpleNamespace(
                model=current_model, provider=current_provider, requested_provider=requested_provider,
                base_url="https://router.example/v1" if current_provider == "custom" else None,
            )
        )
        acp_agent = HermesACPAgent(session_manager=manager)
    ...
    def test_auto_request_keeps_resolved_provider_id(self):
        manager = SessionManager(
            agent_factory=lambda: SimpleNamespace(
                model="anthropic/claude-sonnet-4.5", provider="openrouter", requested_provider="auto", base_url=None
            )
        )
        acp_agent = HermesACPAgent(session_manager=manager)
        state = manager.create_session(cwd="/tmp")
        with patch("acp_adapter.model_catalog._named_custom_provider_catalogs", return_value=[]):
            model_state = acp_agent._build_model_state(state)
        assert isinstance(model_state, SessionModelState)
