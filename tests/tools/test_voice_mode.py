...
class TestSignalDiscrimination:
    """Regression for review feedback on #85793 (arnaudlb, 2026-09-12):
    `rc < 0` used to treat an OOM kill (rc=-9) identically to a deliberate
    SIGTERM stop — silently. The signal source is now discriminated for
    observability (SIGTERM → debug 'interrupted', other signals → warning
    'possible OOM kill'), while the no-fall-through invariant is kept for
    ALL negative rc: re-spawning another player after a hard kill or a
    cross-process stop would layer audio / hammer a machine under memory
    pressure."""
    ...
    def test_sigkill_rc_warns_and_does_not_fall_through(
        ...
        class _KilledProc(TestPlaybackArbitration._FakeProc):
            def wait(self, timeout=None):
        ...
        monkeypatch.setattr("subprocess.Popen", _fake_popen)
        with caplog.at_level(logging.WARNING, logger="tools.voice_mode"):
            result = vm.play_audio_file(sample_wav)

        assert result is False
        # No fall-through: afplay only — ffplay must NOT have been spawned.
        assert len(spawns) == 1 and spawns[0][0] == "afplay"
    ...
    def test_sigterm_rc_does_not_warn(self, monkeypatch, sample_wav, caplog):
        ...
        class _StoppedProc(TestPlaybackArbitration._FakeProc):
            def wait(self, timeout=None):
                self.returncode = -15  # SIGTERM
                return -15

        monkeypatch.setattr("subprocess.Popen", lambda cmd, **kw: _StoppedProc())
        with caplog.at_level(logging.WARNING, logger="tools.voice_mode"):
            result = vm.play_audio_file(sample_wav)

        assert result is False
        assert not any("OOM" in r.message for r in caplog.records)


class TestAplayFormatGate:
    """#109029: on Linux, aplay cannot decode MP3/OGG — it plays raw bytes as
    PCM (white noise) and exits 0, looking like success. The candidate list
    must only offer aplay for ALSA-decodable containers. The candidate
    builder takes the path as data, so the invariant is host-independent."""

    @pytest.mark.parametrize("ext", [".mp3", ".ogg", ".m4a", ".opus"])
    def test_aplay_not_offered_for_non_pcm_containers(self, ext, monkeypatch):
        import tools.voice_mode as vm
        ...
        assert any(c[0] == "ffplay" for c in candidates), (
            "ffplay must remain the decode-any-format fallback"
        )

    @pytest.mark.parametrize("ext", [".wav", ".au", ".voc"])
    def test_aplay_offered_for_alsa_formats(self, ext, monkeypatch):
        import tools.voice_mode as vm
