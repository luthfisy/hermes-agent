...
class TestSendTelegramStandaloneProxy:
    ...
    def test_no_proxy_env_uses_plain_bot(
        ...
        """Without TELEGRAM_PROXY (and no inherited HTTPS_PROXY/etc), Bot() is
        constructed without a proxy: ``request=`` carries only the timeout-tuned
        HTTPXRequest (no ``proxy=`` kwarg), and no ``get_updates_request`` is
        passed since the standalone path never polls. #117795 made the direct
        branch tune timeouts, superseding the old "plain Bot, no HTTPXRequest"
        contract.
        """
        from tools.send_message_tool import _send_telegram
        ...
        # Direct branch: timeout-tuned request, no proxy, no polling request.
        ...
        assert "media_write_timeout" in httpx_request_factory.call_args.kwargs
        bot.send_message.assert_awaited_once()
