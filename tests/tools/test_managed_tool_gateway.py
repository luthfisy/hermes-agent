...
class TestAccessTokenIsExpiring:
    ...
    def test_boundary_skew_is_inclusive(self, monkeypatch):
        """remaining == skew → True (the <= arm); a hair past it → False."""
        ...
        monkeypatch.setattr(managed_tool_gateway, "datetime", FrozenDateTime)
        boundary = (fixed + timedelta(seconds=120)).isoformat()
        assert _access_token_is_expiring(boundary, 120) is True
        just_past = (fixed + timedelta(seconds=120, microseconds=1)).isoformat()
        assert _access_token_is_expiring(just_past, 120) is False
