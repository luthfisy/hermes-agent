...
class TestWriteFileValidatedBeforeGate:
    ...
    def test_batch_shape_is_rejected_and_never_staged(self):
        # The advertised call shape: the whole batch is staged as ONE pending write,
        # so an impossible path in any op must stop it before the gate.
        from tools import write_approval as wa
        with self._staging_gate(), patch.object(
            wa, "stage_write", return_value={"id": "test-pending-1"}
        ) as mock_stage:
            result = json.loads(skill_manage(action="", name="", operations=[
                {"action": "write_file", "name": "some-skill",
                 "file_path": "references/guide.md", "file_content": "ok"},
                {"action": "write_file", "name": "some-skill",
                 "file_path": "/tmp/notes.txt", "file_content": "hello"},
            ]))
        assert result["success"] is False
        assert "operations[1]" in result["error"]
        assert "Absolute paths are not allowed" in result["error"]
    ...
    def test_batch_shape_with_valid_paths_still_reaches_the_gate(self):
        from tools import write_approval as wa
        with self._staging_gate(), patch.object(
            wa, "stage_write", return_value={"id": "test-pending-1"}
        ) as mock_stage:
    ...
    def test_an_approved_staged_batch_is_revalidated_on_apply(self):
        """Same property for the payload the batch shape stages."""
        from tools.skill_manager_tool import apply_skill_pending
        from tools import write_approval as wa
        with patch.object(wa, "stage_write") as mock_stage:
            result = json.loads(apply_skill_pending({
                "action": "batch",
                "operations": [{"action": "write_file", "name": "some-skill",
                                "file_path": "/tmp/notes.txt", "file_content": "hello"}],
            }))
        assert result["success"] is False
        assert "Absolute paths are not allowed" in result["error"]
