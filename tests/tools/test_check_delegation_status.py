...
verification) onto the post-#102117 decomposed kanban modules. This is a
clean add — ``build_parser`` and ``_cmd_delegation_status`` are re-exported
from ``hermes_cli.kanban`` (siblings ``kanban_parser.py`` / defined inline),
and ``tools.async_delegation`` already carries ``_initialize_schema``,
``_records``/``_records_lock``, and the new ``query_delegation_status`` +
``_set_state_db_path_for_tests`` seam this PR added — no adaptation needed
beyond the original PR's content.
