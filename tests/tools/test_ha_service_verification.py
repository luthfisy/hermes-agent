...
class FakeSession:
    ...
    def request(self, method, url, **kwargs):
        # Upstream's _api_json uses session.request(method, ...) rather than
        # the get/post shortcuts; delegate to the same response queue so the
        # service-call POST is served from _responses[0].
