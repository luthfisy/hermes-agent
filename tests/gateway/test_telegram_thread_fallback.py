...
def test_telegram_reply_to_forwarded_bot_message_uses_origin_author():
    import plugins.platforms.telegram.adapter as telegram_mod

    adapter = _make_adapter()
    bot_user = SimpleNamespace(id=777, full_name="Hermes Bot", username="hermesbot", is_bot=True)
    adapter._bot = SimpleNamespace(id=777)
    message = SimpleNamespace(
        text="show diff first",
        caption=None,
        ...
        reply_to_message=SimpleNamespace(
            ...
            text="Checked: server is healthy. Next step: restart gateway.",
            caption=None,
            from_user=SimpleNamespace(id=456, full_name="Alice", username="alice", is_bot=False),
            forward_origin=SimpleNamespace(sender_user=bot_user),
        ),
        quote=None,
        message_id=43,
        date=None,
