...
def test_codex_access_token_env_uses_active_profile_secret_scope(monkeypatch):
    from agent.secret_scope import reset_secret_scope, set_secret_scope

    process_token = _codex_backend_jwt()
    scoped_token = _codex_backend_jwt(int(time.time()) + 7200)
    monkeypatch.setenv("CODEX_ACCESS_TOKEN", process_token)
    scope_token = set_secret_scope({"CODEX_ACCESS_TOKEN": scoped_token})
    try:
        assert _read_codex_access_token_env() == scoped_token
    finally:
