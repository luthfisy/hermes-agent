...
Forward-ported from PR #102390 (fix/delegation-completion-mechanical-
verification) onto the post-#102117 decomposed kanban_db.py. Fixture
follows the current tree's convention (test_kanban_complete_live_claim_guard.py):
reset ``_INITIALIZED_PATHS`` before ``init_db`` and connect via
``kanban_db_connect``, not the removed ad-hoc ``kb.connect()`` pattern.
"""
...
def test_matching_worker_prose_without_evidence_cannot_complete(conn):
...
def test_passed_evidence_bound_to_exact_task_and_run_completes(conn):
    task_id, run_id = _running_task(conn)
...
def test_stale_or_invalid_evidence_cannot_complete(conn, overrides, reason):
    task_id, run_id = _running_task(conn)
...
def test_evidence_rejection_is_audited_without_storing_worker_prose(conn):
    task_id, run_id = _running_task(conn)
    with pytest.raises(kb.CompletionEvidenceError):
        kb.complete_task(
            conn,
...
def test_completion_without_evidence_requirement_is_backward_compatible(conn):
    task_id, run_id = _running_task(conn)
