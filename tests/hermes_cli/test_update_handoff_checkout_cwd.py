...
def test_unrelated_import_error_still_fails(monkeypatch, tmp_path):
    checkout, outside, _finder = _layout(tmp_path)
    monkeypatch.chdir(outside)
    monkeypatch.setattr(handoff, "write_handoff", lambda payload: tmp_path / "handoff.json")
    monkeypatch.setattr(
        handoff,
        "post_swap_command",
        lambda path, tail: [sys.executable, "-c", "import this_package_is_not_in_the_tree"],
    )
    monkeypatch.setattr(handoff, "post_swap_child_env", lambda: os.environ.copy())
    monkeypatch.setattr(handoff, "_post_swap_cwd", lambda: str(checkout))
    monkeypatch.setattr(handoff, "_running_from_windows_shim", lambda: False)
    monkeypatch.setattr(sys, "stdin", open(os.devnull, "r"))
...
def test_current_map_imports_without_the_checkout_cwd(tmp_path):
    """A package already visible on PYTHONPATH does not need the hand-off cwd."""
    _checkout, outside, _finder = _layout(tmp_path)
    visible = tmp_path / "visible"
    pkg = visible / "handoff_newpkg"
    pkg.mkdir(parents=True)
    (pkg / "__init__.py").write_text("MARKER = 'sync'\n", encoding="utf-8")
    env = os.environ.copy()
    env["PYTHONPATH"] = str(visible)
    proc = subprocess.run(
        [sys.executable, "-c", "import handoff_newpkg; print(handoff_newpkg.MARKER)"],
        cwd=outside,
        env=env,
        capture_output=True,
        text=True,
