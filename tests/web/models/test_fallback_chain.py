...
async def _add_via_picker(page: Page, provider_name: str, model_name: str) -> None:
    ...
    # The provider list is fetched after the dialog mounts, and a cold model
    # cache makes that slow. Wait for the load to finish rather than racing it.
    ...
    try:
        await expect(provider.first).to_be_visible(timeout=10000)
    except Exception as exc:
