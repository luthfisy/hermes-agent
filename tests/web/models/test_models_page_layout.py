...
async def _go_to_tab(page: Page, tab_name: str) -> None:
    ...
    tab_selectors = {
        ...
        "Mixture of Agents": "[data-testid='models-settings-moa-tab']",
    ...
    tab_content_selectors = {
        ...
        "Mixture of Agents": "[data-testid='moa-tab-panel']",
...
async def test_all_tabs_visible(page: Page):
    ...
    await expect(page.locator("[data-testid='models-settings-moa-tab']")).to_be_visible()
...
@pytest.mark.asyncio
async def test_moa_tab_has_inline_panel(page: Page):
    """Mixture of Agents tab should render its editor inline, not in a modal."""
    await _go_to_tab(page, "Mixture of Agents")
    await expect(page.locator("[data-testid='moa-tab-panel']")).to_be_visible()
    # Inline panel, so no dialog chrome around the editor.
    await expect(page.get_by_role("dialog", name="Configure Mixture of Agents presets")).to_have_count(0)


@pytest.mark.asyncio
async def test_moa_tab_shows_reference_and_aggregator(page: Page):
    """MoA tab should expose the reference models and aggregator sections."""
    await _go_to_tab(page, "Mixture of Agents")
    panel = page.locator("[data-testid='moa-tab-panel']")
    await expect(panel.get_by_text("Reference models", exact=True)).to_be_visible()
    await expect(panel.get_by_text("Aggregator", exact=True)).to_be_visible()
    await expect(panel.get_by_role("button", name="Save", exact=True)).to_be_visible()
