...
class TestPartialPatchFailureAccounting:
    ...
    FMV must reconcile the explicitly-reported landed subset so only the unresolved requested
    targets remain in the failure state — while the call stays error-shaped for CLI display and
    tool-call guardrails (``file_mutation_result_landed`` still requires ``success`` and no
    top-level error).
    """
    ...
    def test_partial_patch_clears_equivalent_absolute_prior_key(self):
        agent = _bare_agent()
        # An earlier attempt failed, recorded under a RELATIVE spelling.
        agent._record_file_mutation_result(
            "patch", {"mode": "replace", "path": "src/app.py", "old_string": "x", "new_string": "y"},
            json.dumps({"error": "not found"}), is_error=True,
        )
        assert "src/app.py" in agent._turn_failed_file_mutations
        # A later partial retry reports the same file landed under an ABSOLUTE path.
    ...
    def test_success_true_with_top_level_error_reconciles_landed_only(self):
        """success:true AND a top-level error is not a complete success; the call stays
        error-shaped, but FMV reconciles the explicitly reported landed paths."""
        agent = _bare_agent()
        args = {"mode": "patch", "patch": "*** Begin Patch\n*** Update File: /tmp/a.md\n*** End Patch\n"}
        result = json.dumps({
            "success": True,
    ...
    def test_landed_paths_fall_back_to_targets_without_a_reported_subset(self):
        result = json.dumps({"success": False, "error": "failed before any file changed"})
        paths = _extract_landed_file_mutation_paths(
            "patch", {"mode": "patch", "patch": "*** Begin Patch\n*** Update File: /tmp/a.md\n*** End Patch\n"}, result,
        )
        assert paths == ["/tmp/a.md"]
    ...
    def test_reconcile_refuses_an_ambiguous_suffix(self):
        """A bare ``app.py`` is a suffix of two landed files — which one is unknowable, so it
        clears nothing and the target stays failed."""
        reported = ["/tmp/one/app.py", "/tmp/two/app.py"]
        assert _reconciles_to_reported_landed("/tmp/one/app.py", reported) is True
        assert _reconciles_to_reported_landed("app.py", reported) is False
        assert _reconciles_to_reported_landed("app.py", ["/tmp/one/app.py"]) is True

    def test_ambiguous_target_stays_failed_on_partial_patch(self):
        agent = _bare_agent()
        args = {
            "mode": "patch",
            "patch": (
                "*** Begin Patch\n"
                "*** Update File: /tmp/one/app.py\n"
                "*** Update File: /tmp/two/app.py\n"
                "*** Update File: app.py\n"
                "*** End Patch\n"
            ),
        }
        result = json.dumps({
            "success": False,
            "error": "later op failed",
            "files_modified": ["/tmp/one/app.py", "/tmp/two/app.py"],
        })
        agent._record_file_mutation_result("patch", args, result, is_error=True)
        assert set(agent._turn_failed_file_mutations) == {"app.py"}
        assert agent._turn_file_mutation_paths == {"/tmp/one/app.py", "/tmp/two/app.py"}
