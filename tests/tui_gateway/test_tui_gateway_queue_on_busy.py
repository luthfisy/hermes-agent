...
def _session(agent=None, **extra):
    return {
        ...
        "transport": None,
        "attached_images": [],
        # upstream's admission layer claims a real registry slot on the first real turn;
        # unit tests stub the lease so prompt.submit never touches the on-disk registry.
        "active_session_lease": types.SimpleNamespace(lease_id="lease", released=False),
        **extra,
    }
...
def _peers(transport):
    """FanoutTransport transports() lists leaf peers in attach order; bare transports are alone."""
    peers = getattr(transport, "transports", None)
    if callable(peers):
        return tuple(peers())
    return (transport,)
...
def test_prompt_submit_duplicate_rehomes_only_matching_queued_source(monkeypatch):
    ...
    assert response is not None
    assert response["result"]["status"] == "duplicate"
    # upstream wraps concurrent live peers in a FanoutTransport: the retry peer rides alongside
    # "ws-current" rather than displacing it. The PR's rehome guarantee is on the queued source.
    assert _peers(session["transport"]) == ("ws-current", "ws-retry")
...
def _single_peer(transport):
    """FanoutTransport peers are the leaf transports; a single-client session is bare.
    Each drain attaches exactly one new peer (the queuer's socket), appended last."""
    peers = getattr(transport, "transports", lambda: [])()
    return peers[-1] if peers else transport
        ...
        fired.append(
            {
                ...
                "sid": sid,
                "text": text,
                # upstream wraps concurrent live peers in a FanoutTransport (session_transports);
                # unwrap to the recording peer so the assertion below names the submitted socket.
                "transport": _single_peer(session["transport"]),
                **kwargs,
            }
        )
        session["running"] = False
...
class _RecordingTransport:
    def __init__(self, completed: threading.Event | None = None):
        # upstream's transport predicates read ``_closed`` (session_reaper._transport_is_dead /
        # session_transports._transport_is_live_peer) to decide live-peer vs fan-out wrapping.
        self._closed = False
...
def test_session_activate_rehomes_dead_queue_item_and_preserves_live_tail(
    ...
    # upstream wraps concurrent live peers in a FanoutTransport (session_transports), so the
    # activate path must keep the pre-existing live peer and carry the new one alongside it.
    assert isinstance(session["transport"], server.FanoutTransport)
    assert session["transport"].contains(activated_transport)
...
def test_reconnect_rehomes_queued_turn_and_routes_all_events_to_live_transport(
    ...
    def _run_with_delta(user_message, **kwargs):
        ...
        return result
    # _invoke_agent inspects the wrapper's signature for persist_user_* params; a bare **kwargs
    # wrapper hides them and the submitted_at/message_id identity would be dropped on drain.
    _run_with_delta.__signature__ = inspect.signature(original_run)
    ...
    try:
        ...
        assert canonical_users[0]["platform_message_id"] == "desktop-reconnect-1"
