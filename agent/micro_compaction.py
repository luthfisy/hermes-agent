...
class MicroCompactionMixin:
    ...
    def _build_micro_summary_prompt(self, existing_summary: str, exchange_text: str) -> List[Dict[str, str]]:
        ...
        user_prompt = (
            ...
            "open questions.\n\n"
            "VERBATIM PATHS: when mentioning a file path, directory, URL, command, or "
            "code identifier, copy it character-for-character from the source. Never "
            "normalize, shorten, relocate, or invent a path; if you cannot reproduce it "
            "exactly, describe the file by its role instead of guessing an address. "
            "NO META-COMMENTARY: never write sentences about the summarizing process "
            "itself (e.g. 'security rules were enforced', 'tool calls returned exit "
            "code 0'). Every sentence must state a fact taken from the source material.\n\n"
            "If and only if an API key, token, password, secret, credential, or "
            "connection string actually appears in the source, replace it with "
            f"[REDACTED] and note that a credential was present; never preserve its "
            "value. If none appears, write nothing about credentials.\n\n"
