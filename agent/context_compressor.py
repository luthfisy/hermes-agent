...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _call_summary_llm(self, prompt: str, prompt_started_at: float, cached_request: Optional[List[Dict[str, Any]]] = None) -> str:
        ...
        ``cached_request`` (cached_main_model mode) replaces the one-shot prompt with the append-only
        [live captured prefix + instruction] request and pins the call to the MAIN runtime — the
        captured KV prefix only exists on the main provider, so task-config routing must not steer
        this call to the auxiliary backend (it would cold-prefill the whole prefix there)."""
        ...
        if cached_request is not None:
            call_kwargs["messages"] = cached_request
            call_kwargs.pop("model", None)
            # Explicit provider/model/base_url/api_key beat auxiliary.compression.* in
            # _resolve_task_provider_model; route_info records the real selected route.
            call_kwargs.update({
        ...
        # Skipped in cached_main_model mode: the prefix KV lives on the MAIN backend, so a pin to a
        # different one defeats the reuse. The pin stays unconsumed for a later auxiliary attempt.
    ...
    def _generate_summary(
        ...
        # cached_main_model mode: build the append-only request from the live
        # captured prefix so the provider reuses its KV slot (near-zero prefill
        # on local llama.cpp / LM Studio). None -> auxiliary path unchanged.
        _cached_request = None
        if getattr(self, "compression_mode", "auxiliary") == "cached_main_model":
            try:
                _cached_request = self._build_cached_prefix_summary_request(
                ...
                _cached_request = None
        try:
            content = self._call_summary_llm(prompt, prompt_started_at, cached_request=_cached_request)
