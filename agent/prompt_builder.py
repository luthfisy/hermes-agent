...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from your board at `$HERMES_KANBAN_DB` (typically "
    "`~/.hermes/kanban/boards/<slug>/kanban.db`, pinned in your environment). Your task id is in "
    ...
    "\n"
    "## Run discipline\n\n"
    "- **A run ends in a terminal kanban transition.** A prose reply — status, plan, promise — is "
    "recorded as a failed run and re-dispatched; only `kanban_complete`, `kanban_block`, or "
    "`kanban_request_review` end a run. The session can be cut off at any API call; uncommitted work is "
    "lost: commit and `kanban_comment` progress once a stage is verifiable, so a cut-off resumes "
    "from the branch.\n"
    "- **Never hold the run waiting on a remote job** (CI lane, pod, build). Detach the wait: "
    "`terminal(background=true, notify_on_complete=true)` or a bounded poll with an explicit deadline; if it "
    "outlives your budget, `kanban_block` with the job id and last observed state. In-band "
    "`sleep` loops burn turns.\n"
    "- **Guard-safe commands:** no `&` backgrounding (use `background=true`), no foreground timeout above "
    "600 s, no long compound one-liners (write a script and run it), never edit agent-instruction files "
    "(AGENTS.md); never restart the gateway.\n"
    "- **Never read, grep, or echo a credential** (config tokens, `~/.git-credentials`). Headless git hangs "
    "or fails headless — use the documented non-interactive push path, or block "
    "naming what is missing.\n"
    "- **Pace against the turn budget:** check acceptance early; near the limit, commit and finish with "
    "what exists, or block naming the remainder.\n"
    "\n"
    "## Orchestrator mode\n\n"
