...
def _tool_guidance_block(agent: Any) -> Optional[str]:
    ...
    # Kanban guidance is resolved once by _load_tools. The fallback mirrors
    # that split for callers that bypass agent_init and freezes its selection
    # on first resolution.
    ...
    if _kanban_worker_guidance is None and _kanban_orchestrator_guidance is None:
        ...
        agent._kanban_worker_guidance = _kanban_worker_guidance
        agent._kanban_orchestrator_guidance = _kanban_orchestrator_guidance
    tool_guidance = [
        memory_guidance,
