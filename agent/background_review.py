...
_SKILL_REVIEW_PROMPT = (
    ...
    "named target once and retry the write once; do not loopdo not loop.\n"
    "Patch mechanics (small models fail here — follow exactly): skill_view the CURRENT file text, "
    "copy old_string VERBATIM from what skill_view returned (never retype from memory, never "
    "paraphrase, never reconstruct from memory of an earlier version), and keep old_string short — "
    "a unique anchor of 1-3 lines, not a whole section or file. One patch op per file per review; "
    "combine multiple lessons into ONE new_string when they extend the same section rather than "
    "issuing many sequential patches. If old_string cannot be found, re-view the file and retry "
    "once, then fall back to a single small append at a section boundary — never rewrite whole "
    "files.\n\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "read-before-write refusal: view the named target once, retry the write once, do not loopdo not loop.\n"
    "Patch mechanics (small models fail here — follow exactly): skill_view the CURRENT file text, "
    "copy old_string VERBATIM from what skill_view returned (never retype from memory, never "
    "paraphrase, never reconstruct from memory of an earlier version), and keep old_string short — "
    "a unique anchor of 1-3 lines, not a whole section or file. One patch op per file per review; "
