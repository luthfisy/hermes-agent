...
def _maybe_mirror_cron_delivery(
    ...
    try:
        ...
        # removed). A user-role turn merges safely on the per-request API copy through
        # ``drop_thinking_only_and_merge_users`` for strict providers, while remaining a distinct
        # canonical source message. The prefix preserves the "this came from cron" context that the
        # dropped SQLite mirror metadata would otherwise lose on replay.
