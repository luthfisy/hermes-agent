...
~/.hermes/profiles/<name>/ollama_embed.db   # sub-agent
```
...
> The `ollama_embed.db` SQLite is a **per-agent content store** used by the Ollama embed plugin. The **vector memory search** uses a separate LanceDB at `~/.hermes/lance_memory/` (default agent) or `~/.hermes/profiles/<name>/lance_memory/` (sub-agent). These are two different systems.
...
DB = os.path.expanduser("~/.hermes/ollama_embed.db")  # adjust as needed
