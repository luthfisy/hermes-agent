...
- **Built-in snapshot path default is wrong for profile-scoped setups**: Genie derives all paths from `PROFILE_HOME` (`$HERMES_HOME/profiles/$HERMES_PROFILE`), so snapshots resolve to `/root/.hermes/profiles/<profile>/state-snapshots` — not the legacy `/root/.hermes/state-snapshots`. Overrides come from `skills.config.genie.*` in `config.yaml` or `FILESYSTEM.md`; environment variables are no longer read. If a manifest is missing or stale, genie resolves the profile-scoped path by default.
