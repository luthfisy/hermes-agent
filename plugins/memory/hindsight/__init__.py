...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def _apply_recall_settings(self, cfg: dict) -> None:
        ...
        # Reflect tags are OPT-IN and default to empty = unfiltered (bank-wide
        # reflect stays exactly today's behavior). A reflect can pull far more
        # of the bank than a recall — past the engine's reflect context ceiling
        # it degrades into split synthesis and can time out — so narrowing its
        # evidence is a deliberate choice, never an implicit one. Kept separate
        # from recall_tags so the two scopes can be chosen independently.
