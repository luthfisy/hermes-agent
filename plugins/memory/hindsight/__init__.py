...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "observation_scopes", "description": "How observations are scoped during consolidation: 'combined' (default — one pass over all tags), 'per_tag' (one isolated observation per tag), 'all_combinations' (every tag subset — expensive), 'shared' (one pass over the untagged scope — the mode that still dedupes when tags carry volatile per-session provenance), or a JSON list of tag-lists for explicit custom scopes. Empty uses Hindsight's 'combined' default.", "default": ""},
