...
_BALE_API_BASE_URL = "https://tapi.bale.ai/bot"
_BALE_FILE_BASE_URL = "https://tapi.bale.ai/file/bot"


def _redact_bale_error_text(error: object) -> str:
    """Redact secrets from Bale transport errors before logging or returning them."""
...
def _scoped_gate_env(name: str, default: str = "") -> str:
    """Read a BALE_*/GATEWAY_* authorization gate env var per-profile.
    ...
    can return ANOTHER profile's allowlist (issue #72348, Bale mirror).
...
def _consume_abandoned_task(task: asyncio.Task) -> None:
    ...
    try:
        ...
        logger.debug("Abandoned Bale init task failed after timeout", exc_info=True)
...
def _dump_loop_blocked_diagnostics(timeout: float, grace: float) -> None:
    ...
    """
    logger.warning(
        "[Bale] init deadline (%.0fs) expired but the event loop has not "
...
async def _run_abandon_cleanup(on_abandon) -> None:
    ...
    try:
        ...
        logger.debug("Abandoned Bale init cleanup failed", exc_info=True)
...
async def _shutdown_abandoned_app(app) -> None:
    ...
    try:
        ...
        logger.debug("Abandoned Bale app.shutdown() failed", exc_info=True)
    ...
    for request in requests:
        ...
        try:
            ...
            logger.debug("Abandoned Bale request shutdown failed", exc_info=True)

try:
    # Bale implements the Telegram Bot API, so the maintained
    # python-telegram-bot SDK remains the transport dependency.
    ...
    from telegram.ext import (
        ...
        MessageHandler as BaleMessageHandler,
        ContextTypes,
        TypeHandler,
        filters,
    )
...
try:
    ...
    from .bale_network import BaleFallbackTransport, parse_fallback_ip_env
except ImportError:  # pragma: no cover - direct plugin-loader import
    from bale_ids import normalize_bale_chat_id
    from bale_network import BaleFallbackTransport, parse_fallback_ip_env
from utils import atomic_replace, env_float, env_int

def _bale_platform() -> Platform:
    """Resolve Bale's runtime plugin Platform member after registration."""
    return Platform("bale")

_BALE_IMAGE_EXTENSIONS = {".png", ".jpg", ".jpeg", ".webp", ".gif"}
_BALE_IMAGE_MIME_TO_EXT = {
    "image/png": ".png",
    "image/jpeg": ".jpg",
    "image/jpg": ".jpg",
    "image/webp": ".webp",
...
def _probe_voice_duration_seconds(path: str) -> Optional[int]:
    ...
    Bale only auto-derives a clip's duration from container metadata for
    ...
    and the caller omits ``duration``, falling back to Bale's own
...
def bale_deps_present() -> bool:
    ...
    (``check_bale_requirements``) is registered as ``ensure_deps_fn``
    and runs from ``create_adapter()`` when this returns False (#79812).
    """
    return BALE_AVAILABLE
...
def check_bale_requirements() -> bool:
    ...
    install, re-imports the SDK and flips ``BALE_AVAILABLE`` to True
    so the adapter's class-level type aliases get rebound.
    """
    global BALE_AVAILABLE, Update, Bot, Message, InlineKeyboardButton
    global InlineKeyboardMarkup, LinkPreviewOptions, Application
    global CommandHandler, CallbackQueryHandler, BaleMessageHandler
...
def _escape_mdv2(text: str) -> str:
    """Escape Bale MarkdownV2 special characters with a preceding backslash."""
...
def _separate_chunk_indicator_from_fence(text: str) -> str:
    """Move ``(N/M)`` chunk markers off Bale code-fence lines.
    ...
    line like ````` \\(1/2\\)`` after MarkdownV2 escaping. Bale does not
...
# ---------------------------------------------------------------------------
# Markdown table → Bale-friendly row groups
# ---------------------------------------------------------------------------
# Bale's MarkdownV2 has no table syntax — '|' is just an escaped literal,
# so pipe tables render as noisy backslash-pipe text with no alignment.
# The shared convert_table_to_bullets() in gateway.platforms.helpers handles
# the full conversion (detection + rendering); Bale just calls it.
...
# Bale renders both natively, so injecting Markdown hard breaks inside them
...
# Bale endpoints are unreachable. Bounding start_polling() prevents the
...
# Bale transcodes an uploaded video before it answers sendVideo, so the
...
_POLLING_GENERATION_CONTEXT: ContextVar[Optional[int]] = ContextVar(
    "bale_polling_generation", default=None
)
...
class BaleAdapter(BasePlatformAdapter):
    ...
    - Sending responses with Bale markdown
    ...
    supports_code_blocks = True  # Bale MarkdownV2 renders fenced code blocks
    ...
    # Threshold for detecting Bale client-side message splits.
    ...
    # Bale's edit_message applies MarkdownV2 formatting only on the
    ...
    # Retrying a turn-final edit consumes more of the same Bale flood
    ...
    # A failed final edit can leave Bale clients with only a partial or
    ...
    def message_len_fn(self):
        """Bale measures message length in UTF-16 code units."""
        return utf16_len

    def __init__(self, config: PlatformConfig):
        super().__init__(config, _bale_platform())
        ...
        # the raw agent markdown. Disabled by default so Bale messages stay
        ...
        # platforms.bale.extra.rich_messages: true.  Keep this opt-in:
        # current Bale clients can make rich messages difficult to copy
        ...
        # Rich draft previews use a separate opt-in. Bale macOS / Desktop
        ...
        # Transient Bale sendChatAction failures (network blips, 429/5xx)
        # can happen on every keep-typing tick while the agent is waiting on a
        # long model call. Back off per chat so a short Bale-side outage
        # does not spam the API/logs or burn the keep-typing budget.
        self._bale_typing_cooldown_until: Dict[str, float] = {}
        self._bale_typing_cooldown_seconds: float = self._coerce_float_extra(
            "typing_cooldown_seconds",
            30.0,
            min_value=1.0,
            max_value=300.0,
        )
        # Buffer rapid/album photo updates so Bale image bursts are handled
        # as a single MessageEvent instead of self-interrupting multiple turns.
        self._media_batch_delay_seconds = env_float("HERMES_BALE_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        self._pending_photo_batches: Dict[str, MessageEvent] = {}
        self._pending_photo_batch_tasks: Dict[str, asyncio.Task] = {}
        self._media_group_events: Dict[str, MessageEvent] = {}
        self._media_group_tasks: Dict[str, asyncio.Task] = {}
        # Buffer rapid text messages so Bale client-side splits of long
        ...
        # in ~180ms.  All bounds are conservative for Bale's
        # ~1 edit/s flood envelope.
        self._text_batch_delay_seconds = self._env_float_clamped(
            "HERMES_BALE_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "HERMES_BALE_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
        ...
        # before our enqueue/flush drop-guard runs, so Bale will not
        ...
        # Live @username, refreshed whenever Bale tells us what it is.
        ...
        # up. Bale bots have no real presence/online dot (that's a user-account
        ...
        # Document size cap. Bale's public Bot API caps getFile at 20MB; a
        # locally-hosted bale-bot-api server (configured via extra.base_url)
        ...
        #               This is the default — Bale users found per-tool-call
        #               push notifications too noisy.
        # "all"       — every message triggers a push notification (legacy
        #               behavior; opt-in via display.platforms.bale.notifications).
        ...
        # it again is a no-op that still burns Bale's flood budget (~1
    ...
    def _set_fatal_error(self, code: str, message: str, *, retryable: bool) -> None:
        ...
        if not retryable:
            ...
            if n:
                logger.warning(
                    "[Bale] Non-retryable fatal (%s); discarding %d held inbound message(s)",
                    code,
                    n,
                )
    ...
    def _should_drop_delayed_delivery(self) -> bool:
        ...
        already advanced the polling offset, so Bale will never redeliver.
    ...
    def _hold_inbound_event(
        ...
        *,
        ...
        if self._is_permanent_fatal():
            logger.warning(
                "[Bale] Discarding inbound under non-retryable fatal (%s, %d chars)",
                where,
        ...
        while len(held) >= max_n:
            dropped = held.pop(0)
            logger.warning(
                "[Bale] Held-inbound queue full (%d); dropping oldest (%d chars)",
                max_n,
                len(getattr(dropped, "text", None) or ""),
            )
        held.append(event)
        logger.warning(
            "[Bale] Holding inbound (%s, %d chars, queue=%d)%s",
            where,
            len(getattr(event, "text", None) or ""),
            len(held),
    ...
    async def _redispatch_held_inbound(
        ...
        if self._is_permanent_fatal():
            ...
            if held:
                ...
                held.clear()
                logger.warning(
                    "[Bale] Redispatch aborted; discarded %d held inbound under non-retryable fatal",
                    n,
                )
            return
        ...
        held.clear()
        logger.warning(
            "[Bale] Redispatching %d held inbound message(s)",
            len(events),
        )
        ...
        try:
            for idx, event in enumerate(events):
                ...
                try:
                    ...
                    logger.exception(
                        "[Bale] Failed to redispatch held inbound (%d chars); re-holding",
                        len(getattr(event, "text", None) or ""),
                    )
                    self._hold_inbound_event(
    ...
    def _is_callback_user_authorized(
        ...
        *,
        ...
        """Return whether a Bale inline-button caller may perform gated actions."""
        ...
        if callable(auth_fn):
            try:
                ...
                logger.debug(
                    "[Bale] Falling back to env-only callback auth for user %s",
                    normalized_user_id,
                    exc_info=True,
                )

        allowed_csv = _scoped_gate_env("BALE_ALLOWED_USERS").strip()
        if not allowed_csv:
    ...
    def _source_from_message_for_auth(self, message: Message):
        """Build the same Bale source shape the gateway auth path expects.
    ...
    def _bale_auth_env_configured(self) -> bool:
        """Return True when Bale auth env vars make an early decision safe."""
        keys = (
            "BALE_ALLOWED_USERS",
    ...
    def _should_pass_unauthorized_dm_for_pairing(self, source) -> bool:
        ...
        if callable(behavior_fn):
            try:
                ...
                logger.debug(
                    "[Bale] Failed to resolve unauthorized DM behavior; "
    ...
    def _is_user_authorized_from_message(self, message: Message) -> bool:
        """Check if the sender of a Bale message is authorized.
        ...
        if authorized is None:
            ...
            if has_callback or callable(auth_fn):
                ...
                if not self._bale_auth_env_configured():
                    return True
                decision = (
                ...
                if decision is not None:
                    ...
                    try:
                        ...
                        logger.debug(
                            "[Bale] Falling back to env-only auth for user %s",
                            user_id,
                            exc_info=True,
                        )
    ...
    def _metadata_direct_messages_topic_id(cls, metadata: Optional[Dict[str, Any]]) -> Optional[str]:
        if not metadata:
            return None
        topic_id = metadata.get("direct_messages_topic_id") or metadata.get("bale_direct_messages_topic_id")
    ...
    def _metadata_reply_to_message_id(cls, metadata: Optional[Dict[str, Any]]) -> Optional[int]:
        if not metadata:
            return None
        reply_to = metadata.get("bale_reply_to_message_id")
    ...
    def _is_private_dm_topic_send(
        ...
        if cls._metadata_direct_messages_topic_id(metadata) is not None:
            return bool(
                metadata
                and metadata.get("bale_dm_topic_reply_fallback")
                and cls._metadata_reply_to_message_id(metadata) is not None
            )
        if metadata and metadata.get("bale_dm_topic_created_for_send"):
            return False
        return bool(
            thread_id
            and metadata
    ...
    def _dm_topic_missing_anchor_error() -> str:
        return "Bale DM topic delivery requires a reply anchor; refusing to send outside the requested topic"
    ...
    def _reply_to_message_id_for_send(
        ...
        if reply_to:
            return int(reply_to)
        if metadata and metadata.get("bale_dm_topic_reply_fallback"):
    ...
    def _thread_kwargs_for_send(
        ...
        """Return Bale send kwargs for forum and direct-message topic routing.
        ...
        ``bale_dm_topic_reply_fallback``. Live replies send the private
    ...
    def _message_thread_id_for_typing(cls, thread_id: Optional[str]) -> Optional[int]:
        # Asymmetric with _message_thread_id_for_send on purpose. Bale's
    ...
    def _prune_stale_dm_topic_binding(
        ...
        """Drop the stale ``bale_dm_topic_bindings`` row for a
        topic Bale has confirmed deleted.
        ...
        if removed:
            logger.info(
                "[%s] Pruned stale Bale DM topic binding "
    ...
    def _should_retry_without_dm_topic_reply_anchor(
        ...
        """
        if not (metadata and metadata.get("bale_dm_topic_reply_fallback")):
            return False
        ...
        # instead of a reply anchor. If Bale rejects the topic id, fall
    ...
    async def _send_with_dm_topic_reply_anchor_retry(
        ...
        try:
            ...
            if not self._should_retry_without_dm_topic_reply_anchor(
                ...
                raise
            logger.warning(
                "[%s] Reply target deleted for Bale %s, "
                "retrying without reply/topic anchor: %s",
                self.name,
                media_label,
                _redact_bale_error_text(send_err),
            )
    ...
    def _looks_like_connect_timeout(error: Exception) -> bool:
        """Return True when a Bale TimedOut wraps a connect-timeout.

        A plain Bale TimedOut may mean the request reached Bale and
    ...
    def _looks_like_pool_timeout(error: Exception) -> bool:
        """Return True when a Bale TimedOut wraps an httpx pool timeout.

        PTB converts ``httpx.PoolTimeout`` into ``bale.error.TimedOut`` with
        a message that explicitly states the request was *not* sent
        (``"Pool timeout: All connections in the connection pool are occupied.
        Request was *not* sent to Bale."``). Because the request never left
        the process, re-sending is safe and cannot duplicate -- the opposite of
        a generic TimedOut, which may have reached Bale. We match the
    ...
    def _content_fits_rich_limits(self, content: str) -> bool:
        ...
        not pre-counted; if exceeded Bale returns a BadRequest, which
    ...
    def _bot_supports_rich(self) -> bool:
        ...
        ``bale.Bot.do_api_request`` is a coroutine function; test doubles
    ...
    def _has_bale_desktop_details_math_crash_shape(self, content: str) -> bool:
        """Return True for rich-message details+math content that crashes TDesktop.

        Bale Desktop 6.9.1 can crash while rendering Bot API 10.1 rich
        messages containing math inside a collapsible details block
        (baledesktop/tdesktop#30808). The Bot API accepts the payload, so
    ...
    def _has_bale_desktop_cjk_rich_garble_shape(self, content: str) -> bool:
        """Return True for CJK content that current TDesktop rich drafts garble.

        Bale Mac/Desktop Bot API 10.1 rich-message rendering currently
    ...
    def _needs_rich_rendering(self, content: str) -> bool:
        ...
        Keep ordinary replies on the pre-rich MarkdownV2 path so Bale
    ...
    def _rich_eligible(self, content: str) -> bool:
        ...
        return bool(
            ...
            and not self._has_bale_desktop_details_math_crash_shape(content)
            and not self._has_bale_desktop_cjk_rich_garble_shape(content)
    ...
    def prefers_fresh_final_streaming(
        ...
        Disabled for Bale. The fresh-final path briefly shows two copies of
    ...
    def _is_rich_fallback_error(self, exc: Exception) -> bool:
        ...
        Bale, so we must NOT legacy-resend and risk a duplicate.
        """
    ...
    def _compute_single_send_routing(
        ...
        dm_topic_reply_to_off = (
            ...
            and bool(metadata and metadata.get("bale_dm_topic_reply_fallback"))
        )
    ...
    async def _try_send_rich(
        ...
        try:
            ...
            if self._is_rich_fallback_error(exc):
                ...
                logger.debug(
                    ...
                    self.name, _redact_bale_error_text(exc),
                )
                return None
            # Transient / network / unknown: the request may have reached
            # Bale. Do NOT legacy-resend (duplicate risk); surface a
            ...
            # base retry layer honors Bale's backoff instead of its own
            ...
            safe_error = _redact_bale_error_text(exc)
            logger.warning(
        ...
        if message_id is not None:
            # Bale won't echo rich content in reply_to_message, so remember
    ...
    async def _try_edit_rich(
        ...
        # Bale won't echo rich content for messages that predate the bot's
    ...
    def _record_polling_progress(self, generation: int) -> None:
        ...
        if not self._polling_progress_event.is_set():
            ...
            logger.info(
                "[%s] Bale polling confirmed healthy: getUpdates progressing "
                "(generation %d)",
                self.name,
                generation,
            )
    ...
    def _instrument_polling_request(self, request):
        ...
        read-only`` and the whole Bale connect fails (#64482). It only
    ...
    async def _start_polling_once(
        ...
        *,
        ...
        """
        if getattr(self, "_polling_teardown_started", False):
            raise _PollingLifecycleAbort("Bale polling teardown started")
    ...
    async def _drain_general_connections_after_pool_timeout(self) -> None:
        ...
        async with self._get_general_request_drain_lock():
            ...
            try:
                ...
                logger.warning(
                    "[%s] General request pool drained after Bale pool timeout",
                    self.name,
                )
            except Exception:
                logger.debug(
    ...
    def _schedule_polling_recovery(self, error: Exception, *, reason: str) -> None:
        ...
        A Bale bootstrap failure (deleteWebhook / initial start_polling)
        caused by a transient network error should degrade only the Bale
        ...
        if self._polling_error_task and not self._polling_error_task.done():
            logger.debug(
                "[%s] Bale polling recovery already scheduled; ignoring %s: %s",
                self.name, reason, _redact_bale_error_text(error),
            )
            return
        self._send_path_degraded = True
        logger.warning(
            "[%s] Bale polling degraded (%s); gateway stays alive and will retry. Error: %s",
            self.name, reason, _redact_bale_error_text(error),
        )
        loop = asyncio.get_running_loop()
    ...
    async def _start_polling_resilient(
        ...
        *,
        ...
        if not (self._app and self._app.updater):
            raise RuntimeError("Bale application/updater not initialized")
        ...
        try:
            ...
            if require_progress:
                ...
                try:
                    ...
                    raise OSError(
                        "Bale getUpdates made no progress within "
                ...
                if strict_error and not progress.is_set():
                    raise OSError(
                        "Bale polling errored before first getUpdates "
                        "success during initial connect: "
                        f"{_redact_bale_error_text(strict_error[0])}"
                    ) from strict_error[0]
                if not progress.is_set():
                    raise OSError(
                        "Bale getUpdates did not become ready during initial connect"
                    )
            ...
            if self._looks_like_polling_conflict(err):
                logger.warning(
                    "[%s] Bale polling bootstrap conflict; gateway stays alive "
    ...
    async def _handle_polling_network_error(self, error: Exception) -> None:
        ...
        if attempt > MAX_NETWORK_RETRIES:
            message = (
                "Bale polling could not reconnect after %d network error retries. "
                "Escalating to gateway recovery." % MAX_NETWORK_RETRIES
            )
            logger.error("[%s] %s Last error: %s", self.name, message, _redact_bale_error_text(error))
            self._set_fatal_error("bale_network_error", message, retryable=True)
            await self._handoff_polling_fatal_error()
            return

        delay = min(BASE_DELAY * (2 ** (attempt - 1)), MAX_DELAY)
        safe_error = _redact_bale_error_text(error)
        logger.warning(
            "[%s] Bale network error (attempt %d/%d), reconnecting in %ds. Error: %s",
        ...
        try:
            if not app:
                raise RuntimeError("Bale application was torn down during reconnect")
            ...
            logger.info(
                "[%s] Bale polling restarted after network error (attempt %d); "
            ...
            if getattr(self, "_polling_teardown_started", False):
                return
            safe_retry_error = _redact_bale_error_text(retry_err)
            logger.warning("[%s] Bale polling reconnect failed: %s", self.name, safe_retry_error)
    ...
    async def _polling_heartbeat_loop(self) -> None:
        """Detect dead Bale TCP sockets (CLOSE-WAIT) by periodic probing.

        PTB's long-poll task blocks on epoll waiting for Bale to push an
        ...
        *general* request path (not the getUpdates pool), so a healthy long-poll
        waiting for the 30-second Bale window is never interrupted.  On any
            ...
            try:
                ...
                if recovery_task is not None and not recovery_task.done():
                    ...
                    if recovery_task is not stuck_task_ref:
                        ...
                        logger.error(
                            "[%s] Bale reconnect task wedged for %.0fs with no "
                        ...
                        self._set_fatal_error(
                            "bale_network_error",
                            "Bale reconnect task wedged for %.0fs; forcing "
                ...
                # handle Bale just reported before anything routes on it.
                ...
                if self._looks_like_network_error(probe_err):
                    self._schedule_polling_recovery(probe_err, reason="heartbeat probe")
                    continue
                # Non-connectivity errors (e.g. BaleError 401) are not
    ...
    async def _probe_pending_updates(self, bot, probe_timeout: float) -> None:
        ...
        if getattr(self, "_polling_teardown_started", False):
            return
        # Only meaningful in polling mode; in webhook mode Bale pushes
        ...
        if not getattr(updater, "running", False):
            ...
            logger.warning(
                "[%s] Bale polling heartbeat: updater stopped while in "
            ...
            if self._polling_not_running_count >= 2:
                ...
                if getattr(self, "_polling_teardown_started", False):
                    return
                logger.warning(
                    "[%s] Bale updater is not running (long-poll task "
                ...
                self._polling_error_task = loop.create_task(
                    self._handle_polling_network_error(
                        RuntimeError("Bale updater stopped while in polling mode")
                    )
                )
            return
        self._polling_not_running_count = 0
        ...
        logger.warning(
            "[%s] Bale polling heartbeat: %d update(s) queued but not "
        ...
        try:
            ...
            if not self._looks_like_network_error(probe_err):
                logger.warning(
                    ...
                    " (not retrying): %s",
                    self.name, _redact_bale_error_text(probe_err),
                )
                return
            logger.warning(
    ...
    def _disarm_ptb_retry_loop(self) -> None:
        ...
        ``max_retries=-1`` (retry forever).  When a ``BaleError`` (including
        ...
        *synchronously*, then sleeps and re-checks ``while is_running()`` before
        ...
        Bale returns a fresh 409 — a self-inflicted conflict loop on a
        ~31s cadence.
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        if getattr(self, "_polling_teardown_started", False):
            return
        if self.has_fatal_error and self.fatal_error_code == "bale_polling_conflict":
            return
        ...
        # but its long-poll connection hasn't yet expired on Bale's servers.
        # Bale holds open getUpdates sessions for up to ~30s after the
        # client disconnects, so a new gateway starting immediately will receive
        # a 409 until that server-side session expires.
        #
        # Strategy: stop the local updater, wait long enough for Bale's
        ...
        # Bale server-side getUpdates sessions typically expire within
        ...
        if self._polling_conflict_count <= MAX_CONFLICT_RETRIES:
            logger.warning(
                "[%s] Bale polling conflict (%d/%d) — previous session still "
                "held open on Bale's servers. Waiting %ds for it to expire. "
                "Error: %s",
                self.name, self._polling_conflict_count, MAX_CONFLICT_RETRIES,
                RETRY_DELAY, _redact_bale_error_text(error),
            )
            ...
            if not app:
                raise RuntimeError("Bale application was torn down during conflict reconnect")
            # drop_pending_updates=True tells Bale to terminate any
            ...
            try:
                ...
                logger.info(
                    "[%s] Bale polling restarted after conflict retry %d/%d; "
                    "health pending getUpdates progress",
                    self.name, self._polling_conflict_count, MAX_CONFLICT_RETRIES,
                )
                return
                ...
                if getattr(self, "_polling_teardown_started", False):
                    return
                logger.warning(
                    "[%s] Bale polling retry %d/%d failed: %s. "
                    "Scheduling next attempt.",
                    self.name, self._polling_conflict_count, MAX_CONFLICT_RETRIES,
                    _redact_bale_error_text(retry_err),
                )
        ...
        message = (
            "Bale polling could not recover after %d retries (%ds total wait). "
            "The previous gateway session is still held open on Bale's servers, "
        ...
        logger.error(
            ...
            self.name, message, _redact_bale_error_text(error),
        )
        ...
        _already_fatal = (
            ...
            and self.fatal_error_code == "bale_polling_conflict"
        )
        self._set_fatal_error("bale_polling_conflict", message, retryable=False)
        try:
            ...
            logger.warning(
                "[%s] Failed stopping Bale updater after exhausting conflict retries: %s",
    ...
    async def _create_dm_topic(
        ...
        try:
            ...
            # or we just log and skip — Bale doesn't provide a "list topics" API
            if "topic_name_duplicate" in error_text or "already" in error_text:
                ...
                logger.warning(
                    ...
                    "The user must open the DM with this bot in Bale, tap the bot name "
                ...
                logger.warning(
                    ...
                    self.name, name, chat_id, _redact_bale_error_text(e),
                )
            return None
    ...
    def _persist_dm_topic_thread_id(
        ...
        try:
            ...
            # Navigate to platforms.bale.extra.dm_topics, creating the path
            # when a named delivery target asks us to create a topic that was
            # not predeclared in config.yaml.
            platforms = config.setdefault("platforms", {})
            bale_config = platforms.setdefault("bale", {})
            extra = bale_config.setdefault("extra", {})
        ...
        for chat_entry in self._dm_topics_config:
            ...
            for topic_conf in topics:
                ...
                if thread_id:
                    ...
                    # Send a seed message so the topic is visible in Bale's client.
    ...
    async def _bot_identity_refresh_loop(self) -> None:
        ...
        while True:
            try:
                ...
                logger.debug(
                    "[%s] Bale identity refresh loop iteration failed",
                    self.name, exc_info=True,
                )
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            # Register bot commands so Bale shows a hint menu when users type /
            # List is derived from the central COMMAND_REGISTRY — adding a new
            # gateway command there automatically adds it to the Bale menu.
            try:
                ...
                if not self._bot:
                    return
                # Bale allows up to 100 commands but has an undocumented
                ...
                # platforms.bale.extra.command_menu.
                ...
                # Register for all scopes independently — Bale picks the
                ...
                # Forum topics don't inherit AllGroupChats — Bale resolves
                ...
                if hidden_count:
                    logger.info(
                        "[%s] Bale menu: %d commands registered, %d hidden (over %d limit). Use /commands for full list.",
                        self.name, len(menu_commands), hidden_count, max_commands,
                    )
            except Exception as e:
                logger.warning(
                    "[%s] Could not register Bale command menu: %s",
                    self.name,
                    _redact_bale_error_text(e),
                    exc_info=True,
                )
    ...
    def _register_handlers(self, app) -> None:
        ...
        core handlers.
        """
        app.add_handler(BaleMessageHandler(
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        """Connect to Bale via polling or webhook.

        By default, uses long polling (outbound connection to Bale).
        If ``BALE_WEBHOOK_URL`` is set, starts an HTTP webhook server
        ...
        outage (True — preserve the updates Bale queued while the bot
        ...
        Env vars for webhook mode::
            ...
            BALE_WEBHOOK_URL    Public HTTPS URL (e.g. https://app.fly.dev/bale)
            BALE_WEBHOOK_PORT   Local listen port (default 8443)
            BALE_WEBHOOK_HOST   Bind host (default: unset → dual-stack,
                                    all interfaces IPv4+IPv6)
            BALE_WEBHOOK_SECRET Secret token for update verification
        """
        ...
        try:
            if not self._acquire_platform_lock('bale-bot-token', self.config.token, 'Bale bot token'):
                return False
            ...
            custom_base_url = str(
                ...
                or os.getenv("BALE_API_BASE_URL")
                or _BALE_API_BASE_URL
            ).rstrip("/")
            custom_base_file_url = str(
                self.config.extra.get("base_file_url")
                or os.getenv("BALE_FILE_BASE_URL")
                or _BALE_FILE_BASE_URL
            ).rstrip("/")
            # PTB appends ``/bot<TOKEN>/<method>`` to base_url and
            # ``/bot<TOKEN>/<path>`` to base_file_url. Bale therefore needs
            # the two distinct documented prefixes, not its bare hostname.
            builder = builder.base_url(custom_base_url)
            builder = builder.base_file_url(custom_base_file_url)
            logger.info(
                "[%s] Using Bale API endpoint: %s",
                self.name, custom_base_url,
            )
            # In local-mode bale-bot-api, file_path is an absolute path on the
            ...
            if self.config.extra.get("local_mode"):
                ...
                logger.info("[%s] Using Bale local_mode (read files from disk)", self.name)
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("HERMES_BALE_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("HERMES_BALE_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("HERMES_BALE_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("HERMES_BALE_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("HERMES_BALE_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            disable_fallback = (
                os.getenv("HERMES_BALE_DISABLE_FALLBACK_IPS", "")
                .strip()
                .lower()
                in {"1", "true", "yes", "on"}
            )
            ...
            if disable_fallback:
                fallback_ips = []
            proxy_targets = ["tapi.bale.ai", *fallback_ips]
            proxy_url = resolve_proxy_url("BALE_PROXY", target_hosts=proxy_targets)
            if fallback_ips and not proxy_url and not disable_fallback:
                logger.info(
                    "[%s] Bale fallback IPs active: %s",
                    self.name,
                    ", ".join(fallback_ips),
                )
                ...
                # directly into BaleFallbackTransport so its inner
                ...
                request = HTTPXRequest(
                    **request_kwargs,
                    httpx_kwargs={
                        "transport": BaleFallbackTransport(
                ...
                if disable_fallback:
                    logger.info("[%s] Bale fallback-IP transport disabled via env", self.name)
            ...
            _max_connect = 8
            _init_timeout = _env_float("HERMES_BALE_INIT_TIMEOUT", 30.0)
            ...
            for _attempt in range(_max_connect):
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            f"Bale initialization timed out after {_max_connect} attempts "
                            f"({_init_timeout:.0f}s each) — total connect watchdog "
                            f"deadline ({_init_timeout * _max_connect + 120.0:.0f}s) exceeded. "
                            f"Check network connectivity to tapi.bale.ai "
                            f"or set HERMES_BALE_HTTP_CONNECT_TIMEOUT / "
                            f"HERMES_BALE_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
                        "[%s] Connecting to Bale (attempt %d/%d)…",
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            f"Bale initialization timed out after {_max_connect} attempts "
                            f"({_init_timeout:.0f}s each). Check network connectivity to tapi.bale.ai "
                            f"or set HERMES_BALE_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
            ...
            if webhook_url:
                ...
                # BALE_WEBHOOK_HOST or platforms.bale.extra.webhook_host.
                webhook_host = (
                    os.getenv("BALE_WEBHOOK_HOST", "").strip()
                ...
                try:
                    webhook_secret = (get_secret("BALE_WEBHOOK_SECRET") or "").strip()
                except UnscopedSecretError:
                    webhook_secret = os.getenv("BALE_WEBHOOK_SECRET", "").strip()
                if not webhook_secret:
                    raise RuntimeError(
                        "BALE_WEBHOOK_SECRET is required when "
                        "BALE_WEBHOOK_URL is set. Without it, the "
                        ...
                        "  export BALE_WEBHOOK_SECRET=\"$(openssl rand -hex 32)\"\n\n"
                        "Then register it with Bale when setting the "
                        "webhook via setWebhook's secret_token parameter."
                    )
                from urllib.parse import urlparse
                webhook_path = urlparse(webhook_url).path or "/bale"
                ...
                await self._app.updater.start_webhook(
                    ...
                    allowed_updates=Update.ALL_TYPES,
                    # Webhooks are push-based — Bale does not hold a
                ...
                def _polling_error_callback(error: Exception) -> None:
                    ...
                    if self._looks_like_polling_conflict(error):
                        ...
                        logger.warning("[%s] Bale network _redact_bale_error_text(error), scheduling reconnect: %s", self.name, error)
                        self._polling_error_task = loop.create_task(self._handle_polling_network_error(error))
                        self._background_tasks.add(self._polling_error_task)
                        self._polling_error_task.add_done_callback(self._background_tasks.discard)
                    else:
                        logger.error("[%s] Bale polling _redact_bale_error_text(error): %s", self.name, error, exc_info=True)
                ...
                if not polling_started:
                    logger.warning(
                        "[%s] Connected in degraded Bale mode: gateway is alive, "
            ...
            logger.info("[%s] Connected to Bale (%s mode)", self.name, mode)
            ...
            safe_error = _redact_bale_error_text(e)
            ...
            if self._looks_like_auth_error(e):
                message = (
                    f"Bale bot token rejected: {safe_error}. "
                    "The token is invalid or was revoked — generate a new one "
                    "with @BotFather and update BALE_BOT_TOKEN."
                )
                self._set_fatal_error("bale_auth_error", message, retryable=False)
            else:
                message = f"Bale startup failed: {safe_error}"
                self._set_fatal_error("bale_connect_error", message, retryable=True)
            logger.error("[%s] Failed to connect to Bale: %s", self.name, safe_error)
            return False
    ...
    async def _set_status_indicator(self, online: bool) -> None:
        ...
        # Bale caps short_description at 120 chars.
        text = text[:120]
        try:
            ...
            logger.debug(
                ...
                self.name, text, _redact_bale_error_text(e),
            )
    ...
    async def _cancel_pending_delivery_tasks(self) -> None:
        ...
        if self._is_permanent_fatal():
            ...
            if n_pending:
                logger.warning(
                    "[Bale] Non-retryable fatal teardown; discarding %d pending inbound batch(es)",
                    n_pending,
                )
        else:
    ...
    async def disconnect(self) -> None:
        ...
        if self._app:
            try:
                ...
                if self._app.updater and self._app.updater.running:
                    try:
                        ...
                        logger.warning(
                            ...
                            self.name,
                            _redact_bale_error_text(stop_error),
                        )
                ...
                logger.warning(
                    "[%s] Error during Bale disconnect: %s",
                    self.name, _redact_bale_error_text(e),
                )

        self._app = None
        self._bot = None
        logger.info("[%s] Disconnected from Bale", self.name)
    ...
    async def send(
        ...
        # Skip whitespace-only text to prevent Bale 400 empty-text errors.
        ...
        try:
            ...
            if self._should_attempt_rich(content, metadata=metadata):
                ...
                if rich_result is not None:
                    if rich_result.success:
                        ...
                        # the typing refresh loop; re-arming Bale's ~5s timer
            ...
            if len(chunks) > 1:
                ...
                # MarkdownV2-special parentheses so Bale doesn't reject the
            ...
            for i, chunk in enumerate(chunks):
                ...
                metadata_reply_to = self._metadata_reply_to_message_id(metadata)
                private_dm_topic_send = self._is_private_dm_topic_send(chat_id, thread_id, metadata)
                # reply_to_mode="off" on the existing bale_dm_topic_reply_fallback path
                    ...
                    try:
                        ...
                        try:
                            msg = await self._bot.send_message(
                                chat_id=normalize_bale_chat_id(chat_id),
                                text=chunk,
                                parse_mode=self._outgoing_parse_mode(),
                                reply_to_message_id=reply_to_id,
                                **thread_kwargs,
                        ...
                        if _BadReq and isinstance(send_err, _BadReq):
                            if self._is_thread_not_found_error(send_err) and effective_thread_id is not None:
                                if private_dm_topic_send or (metadata and metadata.get("bale_dm_topic_created_for_send")):
                                    return SendResult(
                                        success=False,
                                        error=str(send_err),
                                        retryable=False,
                                    )
                                # Bale has been observed to return a
                            ...
                            if "message to be replied not found" in err_lower and reply_to_id is not None:
                                if private_dm_topic_send:
                                    safe_send_error = _redact_bale_error_text(send_err)
                                    return SendResult(
                                        success=False,
                        ...
                        # generic timeout may have reached Bale, so don't
                        ...
                        # Bale" -- retrying through the loop is safe and
                        ...
                        if retry_after is not None or "retry after" in str(send_err).lower():
                            if _send_attempt < 2:
                                ...
                                safe_send_error = _redact_bale_error_text(send_err)
                                logger.warning(
                                    "[%s] Bale flood control on send (attempt %d/3), retrying in %.1fs: %s",
                                    self.name,
                                    _send_attempt + 1,
                                    wait,
                                    safe_send_error,
            ...
            # Bale clears the typing state when a new message is delivered,
            ...
            # send returns, so re-arming Bale's ~5s timer here would leave
            # the indicator lingering after the answer with nothing to cancel
            # it (Bale exposes no stop-typing API). See #48678.
            if not (metadata or {}).get("notify"):
                try:
                    await self.send_typing(chat_id, metadata=metadata)
                except Exception:
            ...
            safe_error = _redact_bale_error_text(e)
            logger.error("[%s] Failed to send Bale message: %s", self.name, safe_error)
            err_str = str(e).lower()
            ...
            # TimedOut usually means the request may have reached Bale —
    ...
    async def edit_message(
        ...
        *,
        ...
        """Edit a previously sent Bale message.

        Bale caps single-message text at 4096 UTF-16 codeunits.  Streaming
        ...
        if utf16_len(content) > self.MAX_MESSAGE_LENGTH:
            ...
            # still burns flood budget (Bale counts the request and answers
        ...
        try:
            ...
            try:
                ...
                safe_format_error = _redact_bale_error_text(fmt_err)
                logger.warning(
            ...
            if retry_after is not None or "retry after" in err_str:
                ...
                logger.warning(
                    "[%s] Bale flood control, waiting %.1fs",
                    self.name, wait,
                )
                if wait > 5.0:
                    return SendResult(
            ...
            safe_error = _redact_bale_error_text(e)
            logger.error(
                "[%s] Failed to edit Bale message %s: %s",
                self.name,
                message_id,
                safe_error,
            )
        ...
        try:
                ...
                try:
                    ...
                    if "not modified" not in str(fmt_err).lower():
                        logger.warning(
                            ...
                            self.name, _redact_bale_error_text(fmt_err),
                        )
                        await self._bot.edit_message_text(
                            chat_id=normalize_bale_chat_id(chat_id),
                            message_id=int(message_id),
            ...
            if "not modified" in err_str:
                ...
                logger.error(
                    ...
                    self.name, _redact_bale_error_text(e), exc_info=True,
                )
                return SendResult(success=False, error=_redact_bale_error_text(e))
            ...
            for use_markdown in (True, False) if finalize else (False,):
                try:
                    ...
                    sent_msg = await self._bot.send_message(
                        chat_id=normalize_bale_chat_id(chat_id),
                        text=text,
                        parse_mode=self._outgoing_parse_mode() if use_markdown else None,
                        reply_to_message_id=reply_to_id,
                        **thread_kwargs,
                        **self._link_preview_kwargs(),
                        **self._notification_kwargs(metadata),
                    ...
                    if "reply message not found" in str(send_err).lower():
                        ...
                        try:
                            ...
                            logger.warning(
                                ...
                                self.name, _redact_bale_error_text(_retry_err),
                            )
                            sent_msg = None
                            break
                    if use_markdown:
                    ...
                    logger.warning(
                        ...
                        self.name, _redact_bale_error_text(send_err),
                    )
                    sent_msg = None
                    break
            if sent_msg is None:
                ...
                # delivery and leave the Bale topic clipped after the last
                # delivered chunk.
                logger.warning(
    ...
    async def delete_message(self, chat_id: str, message_id: str) -> bool:
        """Delete a previously sent Bale message.
        ...
        Bale's Bot API ``deleteMessage`` works for bot-posted
        ...
        try:
            ...
            logger.debug(
                "[%s] Failed to delete Bale message %s: %s",
                self.name, message_id, _redact_bale_error_text(e),
            )
            return False
    ...
    def supports_draft_streaming(
        ...
        """Bale supports sendMessageDraft for private chats only.
    ...
    async def send_draft(
        ...
        """Stream a partial message via Bale's native draft API.
        ...
        (Bale has no Bot API to "promote" a draft to a real message — the
            ...
            try:
                ...
                if use_markdown and self._is_bad_request_error(e):
                    logger.debug(
                        ...
                        self.name, chat_id, draft_id, _redact_bale_error_text(e),
                    )
                    continue
                logger.debug(
    ...
    async def _send_message_with_thread_fallback(self, **kwargs):
        """Send a Bale message, retrying once without message_thread_id
        if Bale returns 'Message thread not found'.
    ...
    async def send_update_prompt(
        ...
        try:
            ...
            logger.warning("[%s] send_update_prompt failed: %s", self.name, _redact_bale_error_text(e))
            return SendResult(success=False, error=_redact_bale_error_text(e))
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            logger.warning("[%s] send_exec_approval failed: %s", self.name, _redact_bale_error_text(e))
            return SendResult(success=False, error=_redact_bale_error_text(e))
    ...
    async def send_slash_confirm(
        ...
        try:
            ...
            logger.warning("[%s] send_slash_confirm failed: %s", self.name, _redact_bale_error_text(e))
            return SendResult(success=False, error=_redact_bale_error_text(e))
    ...
    async def send_clarify(
        ...
        try:
            ...
            if choices:
                ...
                # (1, 2, …, Other) to avoid Bale truncation.
                option_lines = "\n".join(
            ...
            if choices:
                # Bale caps callback_data at 64 bytes; keep "cl:<id>:<idx>"
                # short.
                rows = []
            ...
            logger.warning("[%s] send_clarify failed: %s", self.name, _redact_bale_error_text(e))
            return SendResult(success=False, error=_redact_bale_error_text(e))
    ...
    async def send_model_picker(
        ...
        try:
            ...
            logger.warning("[%s] send_model_picker failed: %s", self.name, _redact_bale_error_text(e))
            return SendResult(success=False, error=_redact_bale_error_text(e))
    ...
    async def send_choice_picker(
        ...
        try:
            ...
            logger.warning("[%s] send_choice_picker failed: %s", self.name, _redact_bale_error_text(e))
            return SendResult(success=False, error=_redact_bale_error_text(e))
    ...
    async def _handle_callback_query(
        ...
        if data.startswith("ea:"):
            ...
            if len(parts) == 3:
                ...
                try:
                    ...
                    logger.info(
                        "Bale button resolved %d approval(s) for session %s (choice=%s, user=%s)",
                        count, session_key, choice, user_display,
                    )
                except Exception as exc:
                    logger.error("Failed to resolve gateway approval from Bale button: %s", exc)
                    count = 0
            ...
            if len(parts) == 3:
                ...
                try:
                    ...
                    if result_text and query.message:
                        ...
                        # use message_thread_id; Bale private DM-topic lanes
                            ...
                            send_kwargs.update(
                                self._thread_kwargs_for_send(
                                    ...
                                    str(thread_id),
                                    {
                                        ...
                                        "bale_dm_topic_reply_fallback": True,
                                    },
                                    reply_to_message_id=reply_to_id,
                                    reply_to_mode=self._reply_to_mode
                                )
            ...
            if len(parts) == 3:
                ...
                if resolved:
                    ...
                    try:
                        ...
                        pass
                    logger.info(
                        "Bale clarify button resolved (id=%s, choice=%r, user=%s)",
                    ...
                    await self._notify_clarify_expired(query, user_display)
                    logger.warning(
                        "Bale clarify button: resolve_gateway_clarify returned False (id=%s)",
                        clarify_id,
                    )
            return
        ...
        try:
            ...
            logger.info("Bale update prompt answered '%s' by user %s",
    ...
    def _bale_media_too_large_note(self, label: str, file_size: Any, max_bytes: int) -> str:
        ...
        return (
            f"[Bale {label} skipped: file size {size_text} exceeds the "
            f"{limit_mb} MB limit. Ask the user to send a smaller file.]"
        )

    def _bale_media_size_allowed(self, source: Any, label: str) -> tuple[bool, Optional[str]]:
        """Validate Bale media size before downloading into memory."""
        ...
        if size <= max_bytes:
            return True, None
        return False, self._bale_media_too_large_note(label, size, max_bytes)
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        """Send audio as a native Bale voice message or audio file."""
        if not self._bot:
            return SendResult(success=False, error="Not connected")
        ...
        try:
            ...
            # Compute duration locally — Bale drops it for long clips
                ...
                if ext in {".ogg", ".opus"}:
                        ...
                        try:
                            ...
                            if (_cap_parse_mode is not None
                                ...
                                logger.warning(
                                    ...
                                    "retrying plain: %s",
                                    self.name,
                                    _redact_bale_error_text(_cap_error),
                                )
                    ...
                    if msg is None:
                        raise _last_parse_error or RuntimeError(
                            "Bale send_voice failed for all caption variants"
                        )
                elif ext in {".mp3", ".m4a"}:
                    # Bale's Bot API sendAudio only accepts MP3 / M4A.
                    ...
                    # Formats Bale can't play natively (.wav, .flac, ...)
            ...
            logger.error(
                "[%s] Failed to send Bale voice/audio, falling back to base adapter: %s",
                self.name,
                _redact_bale_error_text(e),
                exc_info=True,
            )
    ...
    async def send_multiple_images(
        ...
        """Send a batch of images natively via Bale's media group API.

        Bale's ``send_media_group`` bundles up to 10 photos/videos into
        ...
        # Chunk into groups of 10 (Bale's album limit)
        CHUNK = 10
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            try:
                ...
                logger.warning(
                    ...
                    self.name, chunk_idx + 1, len(chunks), _redact_bale_error_text(e),
                    exc_info=True,
                )
    ...
    async def send_image_file(
        ...
        **kwargs,
        ...
        """Send a local image file natively as a Bale photo."""
        if not self._bot:
            return SendResult(success=False, error="Not connected")
        ...
        try:
            ...
            # files that Bale just refuses as photos (screenshots, extreme
            ...
            if is_dim_error:
                logger.info(
                    "[%s] Image dimensions exceed Bale photo limits, "
                ...
                logger.warning(
                    "[%s] Failed to send Bale local image as photo, "
            ...
            try:
                ...
                logger.error(
                    "[%s] Failed to send Bale local image as document, "
    ...
    async def send_document(
        ...
        **kwargs,
        ...
        """Send a document/file natively as a Bale file attachment."""
        if not self._bot:
            return SendResult(success=False, error="Not connected")
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        """Send a video natively as a Bale video message."""
        if not self._bot:
            return SendResult(success=False, error="Not connected")
    ...
    async def send_image(
        ...
        """Send an image natively as a Bale photo.
        ...
        try:
            # Bale can send photos directly from URLs (up to ~5MB)
    ...
    async def send_animation(
        ...
        """Send an animated GIF natively as a Bale animation (auto-plays inline)."""
        if not self._bot:
            return SendResult(success=False, error="Not connected")
        ...
        try:
            ...
            logger.error(
                "[%s] Failed to send Bale animation, falling back to photo: %s",
                self.name,
                _redact_bale_error_text(e),
                exc_info=True,
            )
    ...
    def _is_transient_typing_error(exc: Exception) -> bool:
        """Return True for Bale typing errors worth cooling down."""
    ...
    def _record_typing_cooldown(self, chat_id: str, exc: Exception) -> None:
        """Suppress Bale typing refreshes for this chat after transient failures."""
        if not hasattr(self, "_bale_typing_cooldown_until"):
            self._bale_typing_cooldown_until = {}
        loop = asyncio.get_running_loop()
        retry_after = getattr(exc, "retry_after", None)
        try:
            delay = float(retry_after) if retry_after is not None else self._bale_typing_cooldown_seconds
        except (TypeError, ValueError):
            delay = self._bale_typing_cooldown_seconds
        delay = max(1.0, min(delay, 300.0))
        self._bale_typing_cooldown_until[str(chat_id)] = loop.time() + delay

    def _typing_in_cooldown(self, chat_id: str) -> bool:
        if not hasattr(self, "_bale_typing_cooldown_until"):
            self._bale_typing_cooldown_until = {}
            self._bale_typing_cooldown_seconds = 30.0
        until = self._bale_typing_cooldown_until.get(str(chat_id))
        if until is None:
            return False
        if asyncio.get_running_loop().time() < until:
            return True
        self._bale_typing_cooldown_until.pop(str(chat_id), None)
        return False
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            ...
            _is_dm_topic = bool(metadata and metadata.get("bale_dm_topic_reply_fallback"))
            ...
            # For DM topic lanes, Bale may reject message_thread_id.
            ...
            logger.debug(
                "[%s] Failed to send Bale typing indicator: %s",
                self.name,
                _redact_bale_error_text(e),
                exc_info=True,
            )
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        """Get information about a Bale chat."""
        if not self._bot:
            return {"name": "Unknown", "type": "dm"}
        
        try:
            chat = await self._bot.get_chat(normalize_bale_chat_id(chat_id))
            ...
            logger.error(
                "[%s] Failed to get Bale chat info for %s: %s",
                self.name,
                chat_id,
                _redact_bale_error_text(e),
                exc_info=True,
    ...
    def format_message(self, content: str) -> str:
        """
        Convert standard markdown to Bale MarkdownV2 format.
        ...
        # Bale accepts Telegram-shaped requests but some clients/API versions
        # render MarkdownV2 escapes literally (``\-``, ``\.``). Plain text is
        # therefore the safe default. Operators that have verified MarkdownV2
        # for their Bale bot can opt in with ``extra.markdown_v2: true``.
        if not self._markdown_v2_enabled():
        ...
        # 0) Rewrite GFM-style pipe tables into Bale-friendly row groups
        ...
        #    (Bale MarkdownV2: **> for expandable start, || to end the quote)
        def _convert_blockquote(m):
            prefix = m.group(1)  # >, >>, >>>, **>, or **>> etc.
            content = m.group(2)
            # Check if content ends with || (expandable blockquote end marker)
            # In this case, preserve the trailing || unescaped for Bale
    ...
    def _markdown_v2_enabled(self) -> bool:
        """Whether this Bale deployment has explicitly opted into MarkdownV2."""
        value = (getattr(self.config, "extra", None) or {}).get("markdown_v2", False)
        if isinstance(value, str):
            return value.strip().lower() in {"1", "true", "yes", "on"}
        return bool(value)

    def _outgoing_parse_mode(self):
        """Use MarkdownV2 only where Bale support was explicitly verified."""
        return ParseMode.MARKDOWN_V2 if self._markdown_v2_enabled() else None

    # ── Group mention gating ──────────────────────────────────────────────

    def _bale_require_mention(self) -> bool:
        ...
        return os.getenv("BALE_REQUIRE_MENTION", "false").lower() in {"true", "1", "yes", "on"}

    def _bale_observe_unmentioned_group_messages(self) -> bool:
        """Return whether skipped unmentioned group messages are stored as context.

        When enabled with ``require_mention``, Bale matches the Yuanbao /
        ...
        if configured is not None:
            if isinstance(configured, str):
                return configured.lower() in {"true", "1", "yes", "on"}
            return bool(configured)
        return os.getenv("BALE_OBSERVE_UNMENTIONED_GROUP_MESSAGES", "false").lower() in {"true", "1", "yes", "on"}

    def _bale_guest_mode(self) -> bool:
        ...
        if configured is not None:
            if isinstance(configured, str):
                return configured.lower() in {"true", "1", "yes", "on"}
            return bool(configured)
        return os.getenv("BALE_GUEST_MODE", "false").lower() in {"true", "1", "yes", "on"}

    def _bale_exclusive_bot_mentions(self) -> bool:
        ...
        if configured is not None:
            if isinstance(configured, str):
                return configured.lower() in {"true", "1", "yes", "on"}
            return bool(configured)
        return os.getenv("BALE_EXCLUSIVE_BOT_MENTIONS", "true").lower() in {"true", "1", "yes", "on"}

    def _bale_free_response_chats(self) -> set[str]:
        raw = self.config.extra.get("free_response_chats")
        if raw is None:
            raw = _scoped_gate_env("BALE_FREE_RESPONSE_CHATS")
        if isinstance(raw, list):
            return {str(part).strip() for part in raw if str(part).strip()}
        return {part.strip() for part in str(raw).split(",") if part.strip()}

    def _bale_free_response_topics(self) -> set[str]:
        ...
        if raw is None:
            raw = _scoped_gate_env("BALE_FREE_RESPONSE_TOPICS")
        if isinstance(raw, list):
            return {str(part).strip() for part in raw if str(part).strip()}
        return {part.strip() for part in str(raw).split(",") if part.strip()}

    def _bale_is_free_response_topic(self, message: Message) -> bool:
        """True when the message's chat/topic pair is in ``free_response_topics``."""
        topics = self._bale_free_response_topics()
        if not topics:
            return False
    ...
    def _bale_allowed_chats(self) -> set[str]:
        ...
        if raw is None:
            raw = _scoped_gate_env("BALE_ALLOWED_CHATS")
        if isinstance(raw, list):
            return {str(part).strip() for part in raw if str(part).strip()}
        return {part.strip() for part in str(raw).split(",") if part.strip()}

    def _bale_group_allowed_chats(self) -> set[str]:
        """Return Bale chats authorized at group scope."""
        raw = self.config.extra.get("group_allowed_chats")
        if raw is None:
            raw = _scoped_gate_env("BALE_GROUP_ALLOWED_CHATS")
        if isinstance(raw, list):
            return {str(part).strip() for part in raw if str(part).strip()}
        return {part.strip() for part in str(raw).split(",") if part.strip()}

    def _bale_observe_allowed_chats(self) -> set[str]:
        ...
        """
        group_allowed = self._bale_group_allowed_chats()
        if not group_allowed:
            return set()
        response_allowed = self._bale_allowed_chats()
        if response_allowed:
            return group_allowed & response_allowed
        return group_allowed

    def _bale_allowed_topics(self) -> set[str]:
        """Return the whitelist of Bale forum topic IDs this bot handles.

        When non-empty, group/supergroup messages from other topics are
        silently ignored. DMs are never filtered by topic. Bale may omit
        ...
        if raw is None:
            raw = _scoped_gate_env("BALE_ALLOWED_TOPICS")
        if isinstance(raw, list):
            return {str(part).strip() for part in raw if str(part).strip()}
        return {part.strip() for part in str(raw).split(",") if part.strip()}

    def _bale_ignored_threads(self) -> set[int]:
        raw = self.config.extra.get("ignored_threads")
        if raw is None:
            raw = _scoped_gate_env("BALE_IGNORED_THREADS")
        ...
        for value in values:
            ...
            try:
                ...
                logger.warning("[%s] Ignoring invalid Bale thread id: %r", self.name, value)
        return ignored
    ...
    def _compile_mention_patterns(self) -> List[re.Pattern]:
        ...
        if patterns is None:
            raw = os.getenv("BALE_MENTION_PATTERNS", "").strip()
            if raw:
                try:
    ...
    def _effective_message_thread_id(cls, message: Message) -> Optional[str]:
        """Return the routable thread id for a Bale message.

        Forum supergroup messages posted in the General topic arrive with
        ``message_thread_id=None`` while Bale itself addresses that topic
        as thread id ``1``.  Ordinary replies are the opposite footgun:
        Bale populates ``message_thread_id`` with a reply-UI anchor id on
    ...
    # Bale bot handles historically had to end in "bot", but collectible
    ...
    def _note_bot_username(self, username: Optional[str]) -> None:
        ...
        if previous:
            logger.info(
                "[%s] Bale bot username changed: @%s -> @%s "
    ...
    def _observe_bot_identity_from_message(self, message: Message) -> None:
        """Learn our own handle from a message Bale says we authored.

        Bale stamps the *current* username on the bot's own outgoing
    ...
    async def _refresh_bot_identity(self, *, force: bool = False) -> None:
        """Re-read the bot's identity from Bale when the cache may be stale.
        ...
        try:
            ...
            logger.debug(
                "[%s] Bale identity refresh failed (keeping @%s): %s",
    ...
    def _extract_bot_mention_usernames(cls, message: Message, self_username: str = "") -> set[str]:
        """Extract explicit Bale bot usernames mentioned in text/captions.
        ...
        for source_text, entities in _iter_sources():
            for entity in entities:
                ...
                entity_text = BaleAdapter._bale_entity_text(source_text, offset, length).strip()
                ...
                # Bale emits /cmd@botname as one bot_command entity, not as
        ...
        # Entity-less fallback for older/client-specific updates. If Bale
    ...
    @staticmethod
    def _bale_entity_text(source_text: str, offset: int, length: int) -> str:
        """Return a Bale entity span using UTF-16 code-unit offsets."""
        if offset < 0 or length <= 0:
            return ""
        try:
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # Bale parses mentions server-side and emits MessageEntity objects
        ...
        for source_text, entities in _iter_sources():
            for entity in entities:
                ...
                if entity_type == "mention" and expected:
                    ...
                    if offset < 0 or length <= 0:
                        continue
                    if self._bale_entity_text(source_text, offset, length).strip().lower() == expected:
                        return True
                    ...
                    # Bale's official group-disambiguation form for slash
                    ...
                    # matches. This is the form Bale's own command menu
                    ...
                    if offset < 0 or length <= 0:
                        continue
                    command_text = self._bale_entity_text(source_text, offset, length)
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Bale groups can contain several Hermes bot profiles. A message like
        ...
        MessageEntity values are preferred, but some Bale clients expose
    ...
    def _is_guest_mention(self, message: Message) -> bool:
        ...
        """
        return self._bale_guest_mode() and self._message_mentions_bot(message)
    ...
    def _should_observe_unmentioned_group_message(self, message: Message) -> bool:
        ...
        if self._is_own_message(message):
            return False
        if not self._bale_observe_unmentioned_group_messages():
            return False
        if not self._is_group_chat(message):
            return False

        thread_id = getattr(message, "message_thread_id", None)
        allowed_topics = self._bale_allowed_topics()
        if allowed_topics:
            topic_id = str(thread_id) if thread_id is not None else self._GENERAL_TOPIC_THREAD_ID
        ...
        if thread_id is not None:
            try:
                if int(thread_id) in self._bale_ignored_threads():
                    return False
            except (TypeError, ValueError):
                return False

        chat_id_str = str(getattr(getattr(message, "chat", None), "id", ""))
        if self._bale_exclusive_bot_mentions() and self._explicit_bot_mentions_exclude_self(message):
            return False

        allowed = self._bale_observe_allowed_chats()
        ...
        if chat_id_str in self._bale_free_response_chats():
            return False
        if self._bale_is_free_response_topic(message):
            return False
        if not self._bale_require_mention():
            return False
    ...
    def _bale_group_observe_shared_source(self, source):
        """Return a chat/topic-scoped source for observed Bale group context."""
        return dataclasses.replace(source, user_id=None, user_name=None, user_id_alt=None)

    def _bale_group_observe_attributed_text(self, event: MessageEvent) -> str:
        user_id = event.source.user_id or "unknown"
        sender = event.source.user_name or user_id
        return f"[{sender}|{user_id}]\n{event.text or ''}"

    def _bale_group_observe_channel_prompt(self) -> str:
        username = self._current_bot_username() or "unknown"
        bot_id = getattr(getattr(self, "_bot", None), "id", None) or "unknown"
        return (
            "You are handling a Bale group chat message.\n"
            f"- Your identity: user_id={bot_id}, @-mention name in this group=@{username}\n"
            "- observed Bale group context may be provided in a separate context-only block "
    ...
    def _apply_bale_group_observe_attribution(self, event: MessageEvent) -> MessageEvent:
        ...
        if not allowed or chat_id_str not in allowed:
            return event
        shared_source = self._bale_group_observe_shared_source(event.source)
        observe_prompt = self._bale_group_observe_channel_prompt()
        ...
        return dataclasses.replace(
            event,
            text=self._bale_group_observe_attributed_text(event),
            source=shared_source,
            channel_prompt=channel_prompt,
        )

    def _media_message_type(self, msg: Message) -> MessageType:
        """Classify a Bale media message into a MessageType."""
        if msg.sticker:
    ...
    async def _cache_observed_media(self, msg: Message, event: MessageEvent) -> None:
        ...
        if not (0 < size <= max_bytes):
            ...
            event.text = self._append_observed_note(
                event.text,
                f"[Observed Bale attachment too large or unverifiable. Maximum: {limit_mb} MB.]",
            )
            logger.info("[Bale] Observed group attachment skipped (size=%s)", file_size)
            return
        ...
        try:
            ...
            logger.warning("[Bale] Failed to cache observed group media: %s", _redact_bale_error_text(exc), exc_info=True)
            return
        ...
        if cached is None:
            ...
            # extension).
            event.text = self._append_observed_note(
                event.text, "[Observed Bale attachment could not be read, not cached.]"
            )
            return
        ...
        logger.info("[Bale] Cached observed group %s at %s", cached.kind, cached.path)
    ...
    async def _cache_replied_media(self, msg: Any, event: MessageEvent) -> None:
        ...
        try:
            ...
            logger.warning("[Bale] Failed to cache replied-to media: %s", _redact_bale_error_text(exc), exc_info=True)
            return
        ...
        logger.info("[Bale] Cached replied-to %s at %s", cached.kind, cached.path)

    def _observed_media_source(self, msg: Message):
        """Return (bale_file_source, filename, mime, default_kind) or Nones."""
        if msg.photo:
    ...
    async def _surface_media_cache_failure(
        ...
        transient httpx.ConnectError to Bale's CDN), the attachment never
        ...
        This (1) replies to the user in Bale so they know to retry, and
        ...
        try:
            ...
            logger.warning(
                "[Bale] Failed to notify user about %s cache failure: %s",
                kind,
                reply_err,
                exc_info=True,
            )
    ...
    def _observe_unmentioned_group_message(
        ...
        try:
            ...
            entry = {
                "role": "user",
                "content": self._bale_group_observe_attributed_text(event),
            ...
            adapter_name = getattr(self, "name", "bale")
            logger.info(
                "[%s] Bale group message observed (no bot trigger): chat=%s from=%s",
                adapter_name,
            ...
            adapter_name = getattr(self, "name", "bale")
            logger.warning("[%s] Failed to observe Bale group message: %s", adapter_name, exc)

    def _is_own_message(self, message: Message) -> bool:
        """Return True when the message was sent by this bot itself.

        In some Bale environments (groups, supergroups where the bot can
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        """Apply Bale group trigger rules.
        ...
        the Bale bot menu (``/command@botname``) or by explicitly
        ...
        #
        # Bale stamps our CURRENT @username on those own-messages and on
        ...
        if thread_id is not None:
            try:
                ...
                logger.warning("[%s] Ignoring non-numeric Bale message_thread_id: %r", self.name, thread_id)
        ...
        allowed = self._bale_allowed_chats()
        ...
        if not self._bale_guest_mode() and self._message_mentions_bot(message):
            return True
    ...
    async def _ensure_forum_commands(self, message) -> None:
        ...
        Forum topics don't inherit AllGroupChats scope — Bale resolves
        ...
        async with self._forum_lock:
            try:
                ...
                logger.warning("[%s] Forum command lazy-registration failed: %s", self.name, _redact_bale_error_text(e))
    ...
    def _effective_update_message(self, update: Update) -> Optional[Message]:
        ...
        Bale exposes channel broadcasts as ``update.channel_post`` rather
    ...
    async def _handle_text_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        Bale clients split long messages into multiple updates.  Buffer
        ...
        if not self._is_user_authorized_from_message(msg):
            logger.warning(
                "[Bale] Blocked unauthorized user %s in chat %s",
        ...
        event = self._apply_bale_group_observe_attribution(event)
    ...
    async def _handle_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await self._cache_replied_media(msg, event)
        event = self._apply_bale_group_observe_attribution(event)
        # Bale clients split messages above 4096 chars into multiple
    ...
    # ------------------------------------------------------------------
    # Text message aggregation (handles Bale client-side splits)
    # ------------------------------------------------------------------
    ...
    def _text_batch_key(self, event: MessageEvent) -> str:
        ...
        raw inbound ``message_thread_id`` Bale may have attached.
        """
    ...
    def _enqueue_text_event(self, event: MessageEvent) -> None:
        ...
        When Bale splits a long user message into multiple updates,
    ...
    async def _flush_text_batch(self, key: str) -> None:
        ...
        Uses a longer delay when the latest chunk is near Bale's 4096-char
        ...
        try:
            ...
            if self._should_drop_delayed_delivery():
                ...
                return
            logger.info(
                "[Bale] Flushing text batch %s (%d chars)",
    ...
    def _photo_batch_key(self, event: MessageEvent, msg: Message) -> str:
        """Return a batching key for Bale photos/albums."""
        from gateway.session import build_session_key
    ...
    async def _flush_photo_batch(self, batch_key: str) -> None:
        ...
        try:
            ...
            if self._should_drop_delayed_delivery():
                ...
                event = None
                return
            logger.info("[Bale] Flushing photo batch %s with %d image(s)", batch_key, len(event.media_urls))
            await self.handle_message(event)
            event = None
        except asyncio.CancelledError:
            if event is not None:
    ...
    async def _handle_media_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not self._is_user_authorized_from_message(update.message):
            logger.info(
                "[Bale] Blocked media from unauthorized user %s in chat %s",
        ...
        # even after Bale's ephemeral file URLs expire (~1 hour).
        if msg.photo:
            try:
                ...
                logger.info("[Bale] Cached user photo at %s", cached_path)
                media_group_id = getattr(msg, "media_group_id", None)
                if media_group_id:
                ...
                logger.warning("[Bale] Failed to cache photo: %s", _redact_bale_error_text(e), exc_info=True)
                await self._surface_media_cache_failure(msg, event, "photo", e)

        # Download voice/audio messages to cache for STT transcription
        if msg.voice:
            try:
                allowed, note = self._bale_media_size_allowed(msg.voice, "voice message")
                if not allowed:
                    event.text = self._append_observed_note(event.text, note or "")
                    logger.info("[Bale] Skipped oversized user voice (size=%s)", getattr(msg.voice, "file_size", None))
                    await self.handle_message(event)
                    return
                ...
                logger.info("[Bale] Cached user voice at %s", cached_path)
            except Exception as e:
                logger.warning("[Bale] Failed to cache voice: %s", _redact_bale_error_text(e), exc_info=True)
                await self._surface_media_cache_failure(msg, event, "voice message", e)
        elif msg.audio:
            try:
                allowed, note = self._bale_media_size_allowed(msg.audio, "audio file")
                if not allowed:
                    event.text = self._append_observed_note(event.text, note or "")
                    logger.info("[Bale] Skipped oversized user audio (size=%s)", getattr(msg.audio, "file_size", None))
                    await self.handle_message(event)
                    return
                ...
                logger.info("[Bale] Cached user audio at %s", cached_path)
            except Exception as e:
                logger.warning("[Bale] Failed to cache audio: %s", _redact_bale_error_text(e), exc_info=True)
                await self._surface_media_cache_failure(msg, event, "audio file", e)

        elif msg.video:
            try:
                allowed, note = self._bale_media_size_allowed(msg.video, "video file")
                if not allowed:
                    event.text = self._append_observed_note(event.text, note or "")
                    logger.info("[Bale] Skipped oversized user video (size=%s)", getattr(msg.video, "file_size", None))
                    await self.handle_message(event)
                    return
                ...
                logger.info("[Bale] Cached user video at %s", cached_path)
            except Exception as e:
                logger.warning("[Bale] Failed to cache video: %s", _redact_bale_error_text(e), exc_info=True)
        ...
        # Download document files to cache for agent processing
            ...
            try:
                ...
                if not ext and doc_mime:
                    ext = _BALE_IMAGE_MIME_TO_EXT.get(doc_mime, "")
                    if not ext:
                ...
                if not doc.file_size or doc.file_size > self._max_doc_bytes:
                    ...
                    logger.info("[Bale] Document too large: %s bytes", doc.file_size)
                    await self.handle_message(event)
                    return

                # Bale may deliver screenshots/photos as documents. If the
                # payload is actually an image, route it through the image cache
                # and batching path instead of rejecting it as a document.
                if ext in _BALE_IMAGE_EXTENSIONS or doc_mime.startswith("image/"):
                    file_obj = await doc.get_file()
                    image_bytes = await file_obj.download_as_bytearray()
                    image_ext = ext if ext in _BALE_IMAGE_EXTENSIONS else _BALE_IMAGE_MIME_TO_EXT.get(doc_mime, ".jpg")
                    try:
                        cached_path = cache_image_from_bytes(bytes(image_bytes), ext=image_ext)
                    except ValueError as e:
                        logger.warning("[Bale] Failed to cache image document: %s", _redact_bale_error_text(e), exc_info=True)
                        event.text = (
                    ...
                    event.message_type = MessageType.PHOTO
                    event.media_urls = [cached_path]
                    event.media_types = [doc_mime if doc_mime.startswith("image/") else _BALE_IMAGE_EXT_TO_MIME.get(image_ext, "image/jpeg")]
                    logger.info("[Bale] Cached user image-document at %s", cached_path)
                ...
                if ext in SUPPORTED_VIDEO_TYPES:
                    ...
                    event.message_type = MessageType.VIDEO
                    logger.info("[Bale] Cached user video document at %s", cached_path)
                    await self.handle_message(event)
                    return

                # NOTE: image-document handling is performed earlier in this
                # function (ext in _BALE_IMAGE_EXTENSIONS or image/* mime),
                ...
                logger.warning("[Bale] Failed to cache document: %s", _redact_bale_error_text(e), exc_info=True)
    ...
    async def _queue_media_group_event(self, media_group_id: str, event: MessageEvent) -> None:
        """Buffer Bale media-group items so albums arrive as one logical event.

        Bale delivers albums as multiple updates with a shared media_group_id.
    ...
    async def _handle_sticker(self, msg: Message, event: "MessageEvent") -> None:
        """
        Describe a Bale sticker via vision analysis, with caching.
        ...
        if cached:
            ...
            logger.info("[Bale] Sticker cache hit: %s", sticker.file_unique_id)
            return
        ...
        try:
            ...
            logger.info("[Bale] Analyzing sticker at %s", cached_path)
            ...
            logger.warning("[Bale] Sticker analysis error: %s", _redact_bale_error_text(e), exc_info=True)
            event.text = build_sticker_injection(
                f"a sticker with emoji {emoji}" if emoji else "a sticker",
                emoji, set_name,
            )
    ...
    def _extract_rich_reply_text(cls, reply_to_message: Any) -> Optional[str]:
        """Return plaintext echoed by Bale's rich_message reply payload."""
        try:
    ...
    def _build_message_event(
        ...
        """Build a MessageEvent from a Bale message.
        ...
        bale_chat_type = str(getattr(chat, "type", "")).split(".")[-1].lower()
        chat_type = "dm"
        if bale_chat_type in {"group", "supergroup"}:
            chat_type = "group"
        elif bale_chat_type == "channel":
            chat_type = "channel"
        ...
        # Prefer Bale's native partial quote (message.quote, TextQuote)
        ...
        if message.reply_to_message:
            ...
            if quote_text:
                ...
                if not reply_to_text:
                    # Prefer Bale's native rich-message echo when present;
    ...
    def _reactions_enabled(self) -> bool:
        ...
        return os.getenv("BALE_REACTIONS", "false").lower() not in {"false", "0", "no"}

    async def _set_reaction(self, chat_id: str, message_id: str, emoji: str) -> bool:
        """Set a single emoji reaction on a Bale message."""
        if not self._bot:
            return False
        try:
            ...
            logger.debug("[%s] set_message_reaction failed (%s): %s", self.name, emoji, _redact_bale_error_text(e))
            return False

    async def _clear_reactions(self, chat_id: str, message_id: str) -> bool:
        """Clear all reactions from a Bale message.
        ...
        try:
            ...
            logger.debug("[%s] clear reactions failed: %s", self.name, _redact_bale_error_text(e))
            return False
    ...
    async def on_processing_complete(self, event: MessageEvent, outcome: ProcessingOutcome) -> None:
        ...
        Unlike Discord (additive reactions), Bale's set_message_reaction
...
#
# Added when the Bale adapter (+ its bale_network satellite) moved from
...
# gateway/run.py, the bale_cfg YAML→env/extra block in gateway/config.py,
# the _setup_bale wizard + _PLATFORMS["bale"] static dict in
# hermes_cli/{setup,gateway}.py, and the standalone sender dispatch in
# tools/send_message_tool.py). Bale uses the generic token connected
# check, so no is_connected override is needed.
# ──────────────────────────────────────────────────────────────────────────


def _resolve_notifications_mode() -> str:
    """Resolve the Bale notification mode (all/important) from env or
    config.yaml display.platforms.bale.notifications, defaulting to
    'important'.  Mirrors the post-construction logic that used to live in
    gateway/run.py::_create_adapter()."""
    mode = os.getenv("HERMES_BALE_NOTIFICATIONS", "")
    if not mode:
        try:
            from gateway.config import load_gateway_config
            from gateway.run import cfg_get
            _gw_cfg = load_gateway_config()
            _raw = cfg_get(_gw_cfg, "display", "platforms", "bale", "notifications")
            if _raw not in {None, ""}:
    ...
    if mode not in {"all", "important"}:
        logger.warning(
            "Unknown bale notifications mode '%s', defaulting to 'important' "
...
def _build_adapter(config):
    """Factory wrapper that constructs BaleAdapter and applies the
    notification mode (preserving the gateway/run.py post-construction step)."""
    # ``gateway.config`` has explicit token hydration only for core platforms.
    # A third-party plugin is enabled through its registry entry, but that
    # generic path deliberately seeds only ``extra`` fields. Resolve the Bale
    # secret here so an env-only BALE_BOT_TOKEN is actually passed to the SDK.
...
def _is_connected(config) -> bool:
    """Bale is connected when a bot token is configured.

    check_bale_requirements() only verifies the python-telegram-bot SDK is
    ...
    Bale on any machine that merely has the SDK installed. Gate on the
    token (env or PlatformConfig.token), matching the generic token check
    Bale had as a built-in.
    """
...
async def _standalone_send(
    ...
    *,
    ...
    """Deliver one message directly to Bale when no live adapter exists."""
    ...
    base_url = str(extra.get("base_url") or os.getenv("BALE_API_BASE_URL") or _BALE_API_BASE_URL).rstrip("/")
    base_file_url = str(extra.get("base_file_url") or os.getenv("BALE_FILE_BASE_URL") or _BALE_FILE_BASE_URL).rstrip("/")
    try:
        from telegram import Bot
        from telegram.constants import ParseMode

        bot = Bot(token=token, base_url=base_url, base_file_url=base_file_url)
        formatted = BaleAdapter.__new__(BaleAdapter).format_message(message)
        send_kwargs: Dict[str, Any] = {
            "chat_id": normalize_bale_chat_id(chat_id),
            "text": formatted,
            "parse_mode": ParseMode.MARKDOWN_V2,
        ...
        for media_path, is_voice in media_files or []:
            ...
            media_kwargs: Dict[str, Any] = {"chat_id": normalize_bale_chat_id(chat_id)}
            if thread_id is not None and str(thread_id) != "1":
                media_kwargs["message_thread_id"] = int(thread_id)
            with open(media_path, "rb") as media:
                if extension in _BALE_IMAGE_EXTENSIONS and not force_document:
                    sent = await bot.send_photo(photo=media, **media_kwargs)
                elif extension in {".mp4", ".mov", ".mkv", ".webm"}:
                    sent = await bot.send_video(video=media, **media_kwargs)
                elif is_voice:
                    sent = await bot.send_voice(voice=media, **media_kwargs)
                elif extension in {".mp3", ".m4a", ".wav", ".ogg"}:
                    sent = await bot.send_audio(audio=media, **media_kwargs)
                else:
                    sent = await bot.send_document(document=media, **media_kwargs)
        if sent is None:
            return {"error": "No deliverable text or media was provided"}
        return {
            "success": True,
            "platform": "bale",
            "chat_id": str(chat_id),
        ...
        return {"error": _redact_bale_error_text(f"Bale send failed: {exc}")}


def interactive_setup() -> None:
    """Prompt for and persist Bale credentials in ``~/.hermes/.env``."""
    # Reuse Hermes' standard env-file writer and allowlist security flow.  The
    # platform key is deliberately ``bale`` so its Telegram managed-bot path
    # (and TELEGRAM_* variables) cannot be selected.
    ...
    _setup_standard_platform(
        {
            ...
            "setup_instructions": [
                "1. Create a bot with @BotFather in Bale and copy its token.",
                "2. Send the bot a message, then enter your Bale numeric user ID below.",
            ],
            "vars": [
                {
                    "name": "BALE_BOT_TOKEN",
                    ...
                    "password": True,
                    "help": "Paste the token supplied by Bale @BotFather.",
                },
                {
                    "name": "BALE_ALLOWED_USERS",
                    "prompt": "Allowed Bale user IDs (comma-separated)",
                    "password": False,
                    "is_allowlist": True,
                    "help": "Enter the Bale numeric user IDs permitted to use this bot.",
                },
            ],
        }
    )
...
def _apply_yaml_config(yaml_cfg: dict, bale_cfg: dict) -> dict | None:
    """Translate config.yaml bale: keys into BALE_* env vars and
    PlatformConfig.extra entries.

    Implements the apply_yaml_config_fn contract (#24849). Mirrors the legacy
    bale_cfg block from gateway/config.py::load_gateway_config(). Env vars
    ...
    # other profile (issue #72348 Bale mirror). They are seeded into
    ...
    if "disable_topic_auto_rename" in bale_cfg:
        extras.setdefault("disable_topic_auto_rename", bale_cfg["disable_topic_auto_rename"])
    ...
    if "mention_patterns" in bale_cfg and not os.getenv("BALE_MENTION_PATTERNS"):
        os.environ["BALE_MENTION_PATTERNS"] = _json.dumps(bale_cfg["mention_patterns"])
    if "exclusive_bot_mentions" in bale_cfg and not os.getenv("BALE_EXCLUSIVE_BOT_MENTIONS"):
        os.environ["BALE_EXCLUSIVE_BOT_MENTIONS"] = str(bale_cfg["exclusive_bot_mentions"]).lower()
    if "allow_bots" in bale_cfg and not os.getenv("BALE_ALLOW_BOTS"):
        os.environ["BALE_ALLOW_BOTS"] = str(bale_cfg["allow_bots"]).lower()
    if "guest_mode" in bale_cfg and not os.getenv("BALE_GUEST_MODE"):
        os.environ["BALE_GUEST_MODE"] = str(bale_cfg["guest_mode"]).lower()
    if "observe_unmentioned_group_messages" in bale_cfg and not os.getenv("BALE_OBSERVE_UNMENTIONED_GROUP_MESSAGES"):
        os.environ["BALE_OBSERVE_UNMENTIONED_GROUP_MESSAGES"] = str(bale_cfg["observe_unmentioned_group_messages"]).lower()
    frc = bale_cfg.get("free_response_chats")
    if frc is not None:
        extras.setdefault("free_response_chats", frc)
        if isinstance(frc, list):
            frc = ",".join(str(v) for v in frc)
        if not _skip_env_bridge and not os.getenv("BALE_FREE_RESPONSE_CHATS"):
            os.environ["BALE_FREE_RESPONSE_CHATS"] = str(frc)
    frt = bale_cfg.get("free_response_topics")
    if frt is not None:
        if isinstance(frt, list):
            frt = ",".join(str(v) for v in frt)
        if not _skip_env_bridge and not os.getenv("BALE_FREE_RESPONSE_TOPICS"):
            os.environ["BALE_FREE_RESPONSE_TOPICS"] = str(frt)
    ac = bale_cfg.get("allowed_chats")
    if ac is not None:
        ...
        if not _skip_env_bridge and not os.getenv("BALE_ALLOWED_CHATS"):
            os.environ["BALE_ALLOWED_CHATS"] = str(ac)
    allowed_topics = bale_cfg.get("allowed_topics")
    if allowed_topics is not None:
        ...
        if not _skip_env_bridge and not os.getenv("BALE_ALLOWED_TOPICS"):
            os.environ["BALE_ALLOWED_TOPICS"] = str(allowed_topics)
    ...
    if ignored_threads is not None:
        ...
        if not _skip_env_bridge and not os.getenv("BALE_IGNORED_THREADS"):
    ...
    _bale_extra = bale_cfg.get("extra") if isinstance(bale_cfg.get("extra"), dict) else {}
    _bale_rtm = (
        bale_cfg["reply_to_mode"] if "reply_to_mode" in bale_cfg
        else _bale_extra.get("reply_to_mode")
    )
    if _bale_rtm is not None and not os.getenv("BALE_REPLY_TO_MODE"):
        _rtm_str = "off" if _bale_rtm is False else str(_bale_rtm).lower()
        os.environ["BALE_REPLY_TO_MODE"] = _rtm_str
    allowed_users = bale_cfg.get("allow_from")
    if allowed_users is not None:
        if isinstance(allowed_users, list):
            allowed_users = ",".join(str(v) for v in allowed_users)
        if not _skip_env_bridge and not os.getenv("BALE_ALLOWED_USERS"):
            os.environ["BALE_ALLOWED_USERS"] = str(allowed_users)
    group_allowed_users = bale_cfg.get("group_allow_from") or _bale_extra.get("group_allow_from")
    if group_allowed_users is not None:
        ...
        if not _skip_env_bridge and not os.getenv("BALE_GROUP_ALLOWED_USERS"):
            os.environ["BALE_GROUP_ALLOWED_USERS"] = str(group_allowed_users)
    group_allowed_chats = bale_cfg.get("group_allowed_chats") or _bale_extra.get("group_allowed_chats")
    if group_allowed_chats is not None:
        ...
        if not _skip_env_bridge and not os.getenv("BALE_GROUP_ALLOWED_CHATS"):
    ...
    for _key in ("guest_mode", "disable_link_previews", "observe_unmentioned_group_messages", "free_response_topics"):
        if _key in bale_cfg:
            extras.setdefault(_key, bale_cfg[_key])
    # Pass through bale-specific extra keys (e.g. base_url proxy override),
    ...
    for _k, _v in _bale_extra.items():
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        check_fn=bale_deps_present,
        ensure_deps_fn=check_bale_requirements,
        is_connected=_is_connected,
        required_env=["BALE_BOT_TOKEN"],
        install_hint="Run `hermes setup` to install Bale support.",
