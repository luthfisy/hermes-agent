"""Helpers for Bale Bot API chat identifiers.

Bale's Bot API accepts a ``chat_id`` in two forms: a numeric ID (an int,
...
# Bale usernames are 5-32 chars: letters, digits, underscores, with a
# leading "@". (Bale also permits 4-char usernames for some legacy/official
...
def normalize_bale_chat_id(chat_id: Any) -> Union[int, str]:
    ...
    Bale's Bot API accepts both, so this never raises on a username the way
...
def looks_like_bale_username(chat_id: Any) -> bool:
    """True when the value is an ``@username``-format Bale chat identifier."""
