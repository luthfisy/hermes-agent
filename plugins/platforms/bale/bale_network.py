"""Bale-specific network helpers.

Provides a hostname-preserving fallback transport for networks where
tapi.bale.ai resolves to an endpoint that is unreachable from the current
host. The transport keeps the logical request host and TLS SNI as
tapi.bale.ai while retrying the TCP connection against one or more fallback
IPv4 addresses.
"""
...
# DNS-over-HTTPS providers used to discover Bale API IPs that may differ
...
# Last-resort IPv4 Bale Bot API endpoints in 149.154.160.0/20
...
SEED_FALLBACK_IPS: list[str] = ["2.189.68.126"]
_UNSET = object()
...
class BaleFallbackTransport(httpx.AsyncBaseTransport):
    """Reach Bale Bot API via known IPv4 literals first, hostname last.

    Requests still target https://tapi.bale.ai/... logically (Host + SNI
    ...
    ``curl --resolve tapi.bale.ai:443:<ip>``. The dual-stack hostname
    ...
    def _attempt_order(self) -> list[Optional[str]]:
        ...
        A blackholed IPv6 path to ``tapi.bale.ai`` never errors — Happy
                    ...
                    async with self._sticky_lock:
                        if self._sticky_ip is _UNSET or self._sticky_ip != ip:
                            ...
                            if ip is not None:
                                ...
                                log(
                                    "[Bale] Using sticky IPv4 Bale API path %s "
                ...
                if self._sticky_ip is not _UNSET and ip == self._sticky_ip:
                    async with self._sticky_lock:
                        if self._sticky_ip is not _UNSET and self._sticky_ip == ip:
                            ...
                            logger.warning(
                                "[Bale] Sticky Bale path %s failed; "
                                "re-walking IPv4 literals before the hostname",
                                ip if ip is not None else "tapi.bale.ai",
                            )
                if ip is None:
                    await self._reset_primary(transport)
                    logger.warning(
                        "[Bale] Dual-stack tapi.bale.ai path failed (%s)",
                        exc,
                    )
                    continue
...
def _resolve_system_dns() -> set[str]:
    """Return the IPv4 addresses that the OS resolver gives for tapi.bale.ai."""
    try:
...
async def discover_fallback_ips() -> list[str]:
    """Auto-discover Bale API IPs via DNS-over-HTTPS.

    Resolves tapi.bale.ai through Google and Cloudflare DoH and returns all
    ...
    to tapi.bale.ai and a transient primary-path failure should be retried
