...
### Gated mode (authenticated public bind)
...
1. Complete the configured dashboard login flow and establish an authenticated HTTP session. The provider may be OAuth, password, self-hosted OIDC, or another registered dashboard-auth provider.
...
When the dashboard is bound to loopback and the auth gate is inactive, companions may use the ephemeral **`?token=`** query parameter. The token is the process-lifetime `_SESSION_TOKEN` injected into the dashboard SPA (or set via `HERMES_DASHBOARD_SESSION_TOKEN`). Constant-time comparison is used server-side.

`--insecure` does not disable dashboard authentication for a non-loopback bind. Remote companions must use the authenticated-session and single-use-ticket flow; a legacy session token is not a remote-access bypass.
...
- Implement gated auth via an authenticated dashboard session + `POST /api/auth/ws-ticket`.
...
- [ ] Store dashboard session credentials in the platform secure store (Android Keystore, iOS Keychain).
...
- [ ] **Gated auth** - complete the configured dashboard login, mint a ticket via `POST /api/auth/ws-ticket`, connect with `?ticket=`, and confirm single-use enforcement (a second connect with the same ticket fails).
