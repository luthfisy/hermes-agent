...
| `WS` | `/events?since=<event_id>` | Live stream of `task_events` rows. Without `since` the stream starts at the board's current tail (the `/board` snapshot already holds the past); pass `since=<the snapshot's latest_event_id>` to catch up from exactly there, or `since=0` to replay the whole history |
