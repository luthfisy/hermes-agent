...
- **Click a card** (without shift/ctrl) to open a side drawer (Escape or click-outside closes) with:
  ...
  - **Editable description** — markdown-rendered by default (headings, bold, italic, inline code, fenced code, `http(s)` / `mailto:` links, bullet **and numbered** lists, nested lists, blockquotes, horizontal rules), with an "edit" button that swaps in a textarea. A wrapped line continues the item or paragraph above it instead of starting a new one. Tables, setext headings and reference links are not supported. Markdown rendering is a tiny, XSS-safe renderer — every substitution runs on HTML-escaped input, only `http(s)` / `mailto:` links pass through, and `target="_blank"` + `rel="noopener noreferrer"` are always set. The desktop app's kanban renders the same description through the app's own markdown renderer, with links handed to the OS browser.
