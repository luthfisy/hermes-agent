...
`snapshot()` returns every visible interactive control — role, name, value, geometry in viewport CSS px, and occlusion (`covered_by`) — which is what lets one call replace the accessibility tree plus a per-node `DOM.getBoxModel` loop. Default cap is 120 (`truncated` is true when more exist); pass `max_elements` to `snapshot()` to override it, and use `page_changed(snap)` to learn in one call whether the DOM moved, so a re-snapshot is only needed when it did. Password inputs are redacted by the helper; every other field value is reported as the page renders it (`snapshot_table(snap, brief=True)` omits values), and `browser_exec` force-redacts its stdout and stderr before they reach the model. Limits match the upstream payload: open shadow roots, iframes, canvas, and nested scrolling are not covered; file inputs still appear as buttons but choosing a file is out of scope; use `cdp()` for the rest.

The install is additive and reversible: a workspace that already has an agent-written `agent_helpers.py` keeps its contents and gains one marker block (an upgrade rewrites only that block), and anything unexpected is debug-logged rather than failing the call. `tools/browser_exec_page_snapshot.py` owns the payload and its version. It is not the same module as `tools/browser_tool_snapshot.py`, which truncates and redacts a snapshot the browser tools have already produced.
