...
**Bare non-virtual `model` values are honored on the OpenAI-compatible
endpoints.** On `POST /v1/chat/completions` and `POST /v1/responses`, the
request's `model` selects a matching `model_routes` alias or is used directly
when no alias matches. Responses report the effective routed model, rather
than echoing a route alias or request value that did not serve the turn.

Deployments that intentionally route every bare model value to the global
default may opt out explicitly:
...
Requests with an explicit `provider` — and the Hermes-native `/v1/runs` and
session-chat endpoints — always honor the requested model regardless of this
