...
`auxiliary.compression.unload_after_seconds` arms **idle unload of a local compression model** — it is the on/off switch (default `0` = off). On unified-memory or single-GPU boxes a local summary model that loads only to compress can keep its weights resident forever; with a TTL set, Hermes records whether the aux model was already loaded before a summary, and if the compression had to load it, fires the endpoint's unload call `unload_after_seconds` after the **last** completed compression — a newer compression resets the timer. The unload call itself resolves per endpoint, no URL needed: Hermes first reads `unload_url` from the **provider entry** serving that endpoint (`custom_providers` / `providers`), else auto-detects LM Studio (native API + loaded-instance id). For servers it cannot identify (e.g. llama-swap), set `unload_url` on the provider entry once — e.g. `http://localhost:8642/api/models/unload/{model}` — or place a literal `auxiliary.compression.unload_url` (with optional `unload_body`) as a global override; `{model}` and `{instance_id}` are replaced in both. Before firing, Hermes re-probes the endpoint and skips the call for a model that is verifiably unloaded, never evicts the model while a summary is mid-flight, and never arms for the model/route serving the conversation itself.
