...
dashboard:
  ...
  pty_attached_idle_minutes: 10.0 # Reap a terminal whose attached client has been silent this long (0 = never)
...
- `pty_attached_idle_minutes` (default `10`) — how long a dashboard terminal may go without a single frame from its attached client before the reaper closes the PTY and the tab reconnects to a fresh one. The chat client keepalives every 20 s, so silence this long means the tab is gone (closed laptop, dropped tunnel, killed browser); without the bound such a terminal stayed open — along with its child's threads — until the server restarted. Set `0` to restore the old never-reap-attached behaviour, or raise it if a client legitimately stalls for long periods (a sleeping laptop that wakes and reconnects automatically is unaffected, because reconnecting cancels the timer before it fires).
