...
`interim_assistant_messages` is gateway-only. When enabled, Hermes sends completed mid-turn assistant updates as separate chat messages on most platforms. On Matrix those updates accumulate into one editable event (`m.replace`) instead of a new timeline root per sentence. This is independent from `tool_progress` and does not require gateway streaming.
