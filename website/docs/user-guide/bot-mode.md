...
- **Several Desktops can share one gateway.** Each Desktop publishes its own roster to a gateway it connects to and drains only the messages addressed through it, so one person's Desktop never delivers a teammate's message onto its own machine. Connection ids are private to each Desktop's **Settings → Connections** (every Desktop calls its own machine the same thing), so when two Desktops publish different machines under one id a bot is told the target is ambiguous instead of having its message delivered to whichever machine answered first.
