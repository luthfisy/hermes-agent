...
- **Enter-to-send or multiline-first** — by default Enter sends and Shift+Enter inserts a newline. Turn off **Settings → Chat → Enter Sends Message** (`desktop.composer.enter_sends: false` in `config.yaml`) for multiline-first input: Enter inserts a newline, Cmd/Ctrl+Enter sends (or queues a follow-up while a turn is running), and Shift+Enter steers the running turn. The shortcuts panel and the `?` help hint follow whichever mode is active. Useful for IME-heavy writing and long structured messages.
