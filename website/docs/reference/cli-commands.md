...
| `diff [key] [--json] [--raw]` | Tree diff of the built-in defaults (`DEFAULT_CONFIG`) vs the effective user config (`config.yaml` + managed overlay + `${VAR}` expansion, no defaults merged), in the `config get` tree style with git's colors: `~ key: <user value> (default: <schema value>)` for values changed from the default, `+ key: <value>` for keys the schema does not define. Unchanged keys and empty branches are pruned. An optional dotted `key` scopes the diff to one subtree (`hermes config diff agent`), like `git diff <path>`; a path present in neither layer exits 1. `--json` emits `{changed, user_only}` maps keyed by dotted path; `--raw` prints credential-shaped values unmasked (masked by default, same rules as `get`). |
