...
| `HERMES_DESKTOP_IGNORE_EXISTING` | Set to `1` to make Desktop ignore every discovered local runtime (the managed install at `HERMES_HOME`, a `hermes` on `PATH`, the system-python `hermes_cli` module) during backend resolution, so no local backend is started; explicit `HERMES_DESKTOP_HERMES_ROOT`, development-source, and `HERMES_DESKTOP_HERMES` deployment overrides still take precedence. Equivalent to `hermes desktop --ignore-existing`. |
