...
| `HERMES_TUI_TERMUX_MODE` | Override Termux-focused defaults only when Termux is detected via `TERMUX_VERSION` or the Termux `PREFIX` path. Defaults to on in Termux; use `HERMES_TUI_TERMUX_MODE=0` to opt out. Accepts `1`/`0`, `true`/`false`, `yes`/`no`, or `on`/`off`. Has no effect outside detected Termux. |
| `HERMES_TUI_INLINE` | Set inline rendering with `1`/`0` (also `true`/`false`, `yes`/`no`, or `on`/`off`). Inline mode uses the primary terminal buffer, preserving output in scrollback; otherwise the TUI uses the alternate screen. Defaults to the effective Termux mode, including its opt-out. |
| `HERMES_TUI_MOUSE_TRACKING` | Set the boot-time mouse default with `1`/`0` (also `true`/`false`, `yes`/`no`, or `on`/`off`), ahead of the legacy `HERMES_TUI_DISABLE_MOUSE` toggle. Without an override, tracking is off in Termux mode and on otherwise unless the legacy toggle is truthy. The `display.mouse_tracking` config and `/mouse` command can adjust tracking after startup. |
