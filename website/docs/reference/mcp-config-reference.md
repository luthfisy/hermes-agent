...
| `transport` | string | HTTP | Set to `sse` to use the SSE transport instead of Streamable HTTP. Pin it for SSE-only servers (a POST answered with `405`) so every startup skips the auto-fallback's failed first connect — see [Streamable HTTP vs SSE](../user-guide/features/mcp.md#streamable-http-vs-sse) |
