...
| `/stop` (alias: `/i`) | Kill all running background processes |
...
| `/stop` (alias: `/i`) | Kill all running background processes and interrupt the running agent. |
...
| `/steer <prompt>` (alias: `/s`) | Inject a message after the next tool call without interrupting — the model picks it up on its next iteration rather than as a new turn. |
