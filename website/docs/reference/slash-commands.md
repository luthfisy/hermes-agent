...
| `/btw <question>` | Ask a quick side question **about the current conversation** without interrupting it. A detached fork answers from a read-only snapshot of the transcript and may **look things up** — `read_file`/`search_files` on files the task touched, `web_search`/`web_extract`, `session_search`, `skill_view` — but every mutating tool (terminal, edits, messages, memory, delegation) is denied at dispatch. The live session's history and prompt cache are untouched, and the current turn keeps running. For independent work with a fresh context, use `/bg`. |
...
| `/btw <question>` | Ask a side question about the current conversation without interrupting it. Answered from a transcript snapshot with read-only lookups (files, web, past sessions) allowed; the answer is sent to the chat when ready. |
