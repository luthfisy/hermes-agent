...
[Grok 4.7](https://docs.x.ai/developers/models/grok-4.7.md) is available in the xAI and xAI OAuth pickers even on a fresh, offline install. Hermes includes its 500,000-token context window, native image input, tool calling, and reasoning capabilities without waiting for an external catalog refresh. Existing model selections and explicit context or capability overrides are preserved. The default stale-response timeout has a 300-second floor for Grok 4.7; explicit timeout settings and the existing run-budget cap still take precedence.
