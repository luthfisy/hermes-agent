...
| 场景 | 行为 |
|---------|----------|
| **私信（DM）** | Hermes 响应每一条消息，无需 `@提及`。`reply_mode: off`（默认）时每个私信是一个会话；`reply_mode: thread` 时每条顶层私信开启新会话，该线程内的回复沿用同一会话。 |
| **公开/私有频道** | Hermes 仅在被 `@提及` 时响应。未被提及时，Hermes 忽略消息。 |
| **线程（Thread）** | 若设置 `MATTERMOST_REPLY_MODE=thread`，Hermes 在你的消息下方以线程形式回复。线程上下文与父频道或私信隔离。 |
...
- `reply_mode` 为 `off`（默认）时，每个私信是一个会话
- `reply_mode: thread` 时，频道或私信中的每条顶层消息开启新会话；该帖下的回复共用同一会话
...
| `thread` | Hermes 在你的原始消息下方以线程形式回复。在频道和私信中，每条顶层消息也会开启新会话；回复留在该会话中。 |
