...
## Skills 与 cron 的构建器声明缓存边界

Skill 调用和加载 skill 的 cron job 会把大型静态脚手架（激活说明、展开后的 skill 内容和固定指令标记）与较小的单次运行指令或事件 payload 合并成一条用户消息。如果把整条消息作为一个缓存块，每次 payload 改变都会使整个脚手架失效。

因此，构建器会声明精确的字节边界，而不是让请求层根据分隔符重新推断：

1. `agent.skill_commands.append_user_instruction()` 追加易变指令并返回稳定前缀。
2. Skill 或 cron 构建器仅在该前缀仍是最终消息的真字节前缀时，才通过 `agent.prompt_cache_boundary.register_stable_prefix()` 注册它。如果注入扫描修改了组装后的字节，注册会被跳过，并安全回退为整条消息缓存策略。
3. 对原生 Anthropic 请求，`agent.prompt_caching` 会发出两个文本部分：稳定脚手架带缓存标记，易变尾部不带。规范会话消息仍保持为普通字符串，因此持久化和 provider 故障转移语义不变。

该注册表仅存在于进程内，并限制为 32 个 LRU 条目。当存在多个条目时，它会淘汰最旧的前缀，直到合计大小约为四百万字符；最新前缀始终保留，因此单个异常大的脚手架可以超过这个软字符预算。未命中、重启或淘汰都是安全的：Hermes 会发送原始的未拆分消息。在长交互会话中，只有当该调用仍位于缓存计划最近的端点时才使用拆分形态；轮出后，provider 最多再次摄取一次该前缀。新的 webhook 和 cron 调用始终把 skill 轮次放在最新端点，因此会继续获得预期的缓存命中。

为另一个具有“静态脚手架 + 易变指令”形态的构建器添加支持时，应复用 `append_user_instruction()`，并只在所有清理转换完成后注册。不要从任意 skill 或事件文本中反向解析指令标记：相同字节可能合法地出现在不可信 payload 中。
