...
Logic (kept in sync with contributor-check.yml):
  ...
  - everything else must have ``contributors/emails/<email>`` or a legacy
    AUTHOR_MAP entry in scripts/release_contributors.py
...
def is_mapped(email: str) -> bool:
    ...
    if (REPO_ROOT / "contributors" / "emails" / email).is_file():
        return True
    legacy_map_file = REPO_ROOT / "scripts" / "release_contributors.py"
    try:
        if f'"{email}"' in legacy_map_file.read_text(encoding="utf-8", errors="replace"):
            return True
    except OSError:
        pass
    return False
